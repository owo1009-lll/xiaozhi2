...
4. The method improves target SDR and downstream pitch-diagnosis stability in piano and light-orchestral settings, while exposing failure boundaries under dense unison strings and poor score alignment.
...
### 3.5 Posterior Pitch Selection

Candidate set:

```text
C_t = {f_det(t), f_score(t), 2*f_score(t), f_score(t)/2}
```

Decision:

```text
f_eff(t) = argmax_f P_det(f | x, t)^lambda * P_score(f | S, t)^(1-lambda)
```

This replaces brittle if-else fallback with a principled detector-prior fusion.
...
Apply the mask to the harmonic spectrogram, reconstruct by iSTFT, and report runtime complexity and real-time factor.
