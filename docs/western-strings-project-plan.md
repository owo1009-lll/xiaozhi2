...
- **当前执行状态(2026-07-09):** 已新增只读 readiness 命令 `npm run western:m4-omr-readiness`,用于审计 M2f clean-score intake 是否足以启动 OMR benchmark。当前实测 12/12 条 `currentScorePath` 图片/谱面源文件存在、12/12 条 `requiredCleanScorePath` gold clean score 存在且已 `approved`、12/12 个 `scoreId` 已在 score store 中,输出 `data/experiments/western-strings-m4/omr-readiness.json` 与 `.csv`;`m4OmrBenchmarkDatasetReady=true`。该命令只证明"可开始跑 OMR 引擎并与 gold score 对比",**不证明 OMR 精度达标**,且输出仍固定 `studentGateReady=false` / `runtimeEffect=none`。
...
16. ✅ 2026-07-09 项目级状态命令已接入:`npm run western:project-status` 同时汇总普通上传候选 gate 与 M3+ pitch-behavior 复核状态,输出 `data/experiments/western-strings-project-status.json`。当前结果应显示普通上传 `ordinaryUploadAutoFeedbackReady=false`、M3+ `m3plusAutoFeedbackReady=false`;下一步优先级为先标普通上传 blind batch,再标 M3+ 48 条 pitch-behavior 包。该命令只读,不得替代任何 release gate。
