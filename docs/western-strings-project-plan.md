...
16. ✅ 2026-07-09 项目级状态命令已接入并随当前证据更新:`npm run western:project-status` 同时汇总普通上传候选 gate、confidence pilot + fresh validation eval、M3+ pitch-behavior 复核状态与 M4 OMR benchmark 状态,输出 `data/experiments/western-strings-project-status.json`。当前结果应显示普通上传 `ordinaryUploadAutoFeedbackReady=false`、M3+ `m3plusAutoFeedbackReady=false`、M4 `m4OmrAutoScoreReady=false`。普通上传 confidence validation fresh blind batch 已通过,但 runtime 仍因 `ordinary-auto-gate-disabled-by-default` 保持 fail-closed;`WESTERN_STRINGS_ENABLE_ORDINARY_AUTO_GATE=1` 仅允许在受控 pilot 进程中使用,不得默认长开。M3+ 48 条 pitch-behavior 人工复核已导入,项目状态里的 M3+ label gate 不再阻塞,但它只表示离线模式评估输入齐备,学生端 M3+ 自动反馈仍关闭。当前项目优先级为:先核实普通上传 runtime 特征提取与冻结 RF scorer 的一致性,并排查 pilot coverage=0.5333 / precision=0.9375 vs fresh validation coverage=1.0 / precision=0.90 的操作点漂移;再做 M3+ per-mode precision/unsafe 评估;然后为 M4 准备独立 gold score 以消除 self-comparison。项目状态命令只读,不得替代 release gate。已新增发布阻断命令 `npm run western:project-gate`;它复用同一份状态逻辑,默认要求 ordinary/m3plus/m4 三条轨道全 ready,当前应以非零退出阻断发布,输出 `data/experiments/western-strings-project-gate.json`。`npm run test:western-project-gate` 覆盖项目级 fail-closed、confidence validation 状态、显式 release-flag 阻断与 M4 自比样本不被当作独立 gold。
