...
- 后续任何新条目一律走 C.2.3 登记合同,未过人工确认+许可审查不得进入可反馈集合。

**登记优先级建议**:v1 验收只需覆盖第一档中的 3–5 首(建议 r2-01/r2-06/r3-01 起步:正常+技法+fresh 各一),全目录可随工程进度渐进登记,不阻塞验收。
...
#### C.3.3 晋升门槛(⚖️ 数字冻结需负责人签署,签署后不得因结果回调)
