...
>
> **受控 pilot 机器验证已扩到 5 条独立录音(2026-07-10):** 全曲口径仍不可放行:275 候选 / 33 个模型原始 auto-pass / 11 个严格候选,全曲有效 coverage=4.00%;联合 threshold sweep 证明不能靠放宽参数同时满足 precision≥90% 与 coverage≥20%。但错误高度集中在后续小节,因此新增明确的 **first-measure-only** 受控范围:只有第 1 小节且 confidence≥0.95 才可 auto-pass,其余全部 `review_required`。该范围历史留一录音为 12/12 正确、coverage=25.53%(5 条录音),真实受控 pilot 为 11/11 正确、coverage=26.83%(5 条独立录音),0 known wrong、0 unknown。runtime scope 已在显式 pilot flag 下接线并通过正反单测/临时 smoke;默认学生端仍 fail-closed。`npm run western:controlled-pilot-evidence-audit` 当前 `machinePreflightPassed=true`,`teacherReviewAllowed=true`,含义仅是**允许准备一份全新的、小型盲验包**,不是允许复用旧标签或默认上线。复核包在交给专业教师前必须先由机器验证音频、谱面定位、按钮和范围;后续小节自动化仍未解决。
> **Fresh blind 入场(2026-07-10):** 现有 12 条录音全部已经进入历史标签/复核,不能再用于最终盲审。全新录音、已审 clean MusicXML/MXL 和对应谱面图片/PDF 就位后,使用 `npm run western:fresh-blind-intake-stage -- --recording-id ... --piece-id ... --audio ... --score ... --score-display ... --reviewed-by ...` 原子化登记,不再手改 JSON。该命令先审计临时清单,只有录音 ID/内容哈希、曲目 ID、谱面内容、音频解码、第一小节音符、谱面批准/显示文件/授权全部通过才替换正式 `intake.json`;失败保持旧清单不变。随后必须用 `npm run western:fresh-blind-intake-status` 确认 `readyForMachinePrecheck=true`,否则不得写入受控 intake 或生成教师包。

> **第二轮受控执行(2026-07-15):** 8/8 组新音频、MusicXML 和谱面图片已完成审计与机器分析。导入时暴露并修复了“多小节 MusicXML 被压缩到第 1 小节”的结构缺陷;结构闸门现要求候选音符数、小节数和唯一 note ID 均与源谱一致。`r2-08` 精确 fresh-blind pilot 处理 60 个候选,模型原始 auto-pass=3,但 scoped/self-checked auto-pass 均为 0,因此按 fail-closed 中止且未发布学生反馈。M3+ 新库存为 444 音符/292 review-only 候选。原始 `README-怎么用.md` 已确认 M3 场景数量为错音 5 / 漏音 5 / 拖拍 4,并已用于机器候选搜索;但 `notes.txt` 未提供具体小节,所以精确 recall/precision 仍不得计算。当前结论是“第二轮机器链路完成,发布闸门未过”,不是 V2 默认开放。
> runner 现在会从历史 session 自动排除所有已执行 `recordingId`;机器预检淘汰但未执行的录音可用 `--exclude-recording-id <recordingId>` 传入并随成功 session 固化。任何重复返回都会以 `pilot-reused-recording` 中止,不得计入独立证据。
...
- **M3+ 最小补证入口(2026-07-15):** `npm run western:m3plus-supplemental-scores` 生成 `音频/m3plus-supplemental/` 四条固定音符受控任务:①C4–C5 上行 8 个纯直音负例;②8 个揉弦+8 个颤音独立正例;③8 个装饰音+8 个同音高普通音对照;④8 组滑音+8 个同目标音直音对照。演奏者不需要看谱,只按 README 的固定顺序和文字要求演奏;生成 MusicXML 已真实写入 `vibrato` 文字、`trill-mark`、`mordent`、`glissando` 及 straight/plain 对照。评测改为“谱面技法意图驱动 + 音频执行核验”:逐音 score-intent 固定 `performanceConfirmed=false`,谱面写了技法不等于录音已经正确执行。`npm run western:m3plus-supplemental-status` 当前报告 0/4 录音就绪;录齐后运行 `npm run western:m3plus-supplemental-eval`,先校验谱面标记,再以单调 F0 序列对齐替代线性切窗。定位允许目标附近 `±100 cents` 的真实演奏偏差,但该容忍只用于找窗口,不等于音准合格;模式闸门仍要求每类 precision>=0.90、recall>=0.80、正负例各>=4。机器定位或模式阈值不过时先修特征,不得交教师;机器通过且演奏者确认执行要求后才允许一次有界专业复核,学生端始终 fail-closed。
...
19. ⚠️ **历史失败,已被 P1.1 context-feature 重校准取代:** 2026-07-09 旧 threshold-pool 分层复核为 usable=23 / wrong=36 / uncertain=1,旧规则 selected precision=0.5556,证明当时的 confidence-only RF 不能进入 pilot。该结果继续保留为负证据,但不再代表当前 release candidate;当前是否可进入受控 pilot 只由后续 P1.1 证据、`western:controlled-candidate-confidence-release-audit`、`western:release-review` 和负责人批准共同决定。默认学生端仍不得开启。
...
21. ✅ 2026-07-09/10 P1.1 context-feature 重校准与 fresh validation 已完成。冻结 runtime artifact 为 `models/western-strings/ordinary-upload-confidence-rf-v1/release.json`,当前 threshold=0.8;release audit 报告 pilot precision=0.942857、validation precision=1、runtime-selected threshold-pool precision=1(12 selected / 53 reviewed)。`npm run western:ordinary-monitored-pilot-audit` 已通过且 `teacherReviewNeeded=false`,但 `readyForDefaultEnable=false`:只允许在负责人批准后的单独受控进程中临时开启,默认学生端始终 fail-closed。
