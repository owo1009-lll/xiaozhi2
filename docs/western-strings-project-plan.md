...
>
> **受控 pilot 决策包(2026-07-10,历史审批流程；2026-07-24 当前合同首跑):** 负责人批准与五批一次性离线 pilot 是旧 RF/first-measure 事实,已显式 superseded。当前 `western:release-review` 必须生成 `schemaVersion=2`、使用 `western-ordinary-dynamic-shadow-release-v1`,并带同轮 live ordinary/M3+ evidence projection SHA;decision/preflight 会重新计算该绑定,手写或陈旧绿报告不得通过。新 approval 必须持久化并复核"独立 monitored pilot"与"默认运行时 fail-closed"两项确认。在 r3 接受性证据和两轨独立 `authorizationReady=true` 合同缺任一项时,`western:controlled-pilot-decision`、旧 approval 和旧 start preflight 都必须失败。2026-07-19 上述条件全部满足；2026-07-24 已按当前合同实际完成一个 `completed_safe` review-only 批，40/40 行没有学生自动反馈。执行后物理最新批已重新绑定 M3+ audit/release/decision/preflight；`studentGateReady` 结构性独立,仍为 `false`。
>
...
- **Oemer 几何与像素归因(2026-07-21):** `western:m4-oemer-dewarp-attribution` 使用上述同一 full-page/up2 输入逐页隔离推理，比较 Oemer 原始 staff mask 与其内置 `estimate_coords/griddata/cv2.remap` 后的 mask。5/5 Jacobian 无折叠；可比较 3 页的直线度 p90 均改善，但五线组平均恢复率前后同为 `85.88%`，interline CV p90 反而 5/5 恶化。该结果否定“只是缺 dewarp”的归因，不支持立即重写 TPS；今后的 dewarp 验收必须同时约束直线度 p90 与 interline CV p90 均不恶化，并继续检查五线组恢复率和 Jacobian 折叠，禁止用直线度单指标换取谱线间距畸变。M4a 构建器现已从 MuseScore SVG 语义类自动生成 stem/beam/notehead/barline 同分辨率像素 gold，并写入与已签署 M4a 产品 registry/sidecar 分离的独立 mask registry，避免改变既有验收哈希；r2-01 clean/synthetic-s17 的 Oemer 2px 容差 recall 为 stem `97.37%/96.34%`、notehead `98.02%/98.20%`、barline `93.72%/92.84%`，beam 因 3 个登记页均无正样本而不可评测。该证据只覆盖 render/synthetic，且 stem/barline 来自非排他合并层，只能报告覆盖率，不能外推真照片或报告类别 precision。
- **M4b 解码层止损与数据门槛(2026-07-21):** 22 个 clean measure 失败曲目中，小节线无漏检为 `19/22`，但完全无漏/无多检仅 `4/22`，额外小节线存在于 `17/22`，所以检测误检与后续解码均有责任。进一步用 Verovio SVG 精确语义 mask 绕过 Oemer 检测/谱表恢复/dewarp，29/32 可解码项仍只有 pitch P/R=`59.91%/60.56%`、onset/measure=`2.70%/42.04%`、结构严格 `0/29`；按分叉规则实现的 clean-only 硬拍号 DP 仅能配平 `354/2147` 小节、实际改写 `0` 小节，onset 仅到 `10.59%`，32 首严格通过 `0`，触发 clean 80% 止损。复用 5 张真照片的唯一一次符号级诊断为 notehead/stem/barline-boundary recall=`98.27%/98.23%/84.71%`，但真照片像素级三类 recall 因无同版坐标 mask gold 不可评；该 5 页已登记为只读诊断，永久禁止调参。此后不再投入 dewarp/TPS、新 detector 或 solver；下一结论必须等待至少 30 页、6 版式、3 设备 fresh-blind 真照片+逐页结构标注，否则冻结 M4b 开放域研究线。M4a 已签署 registry/sidecar 不受影响。
...
16. ✅ 项目级状态命令已更新到当前证据:`npm run western:project-status` 输出 `ordinaryDynamicShadow` live preflight、r3 acceptance、authorization 与历史 RF supersession,同时汇总 M3+ 和 M4。M3+ 会固定并重哈希 5 个规范 source bindings(含 evaluator),再重审物理 JSONL 尾批、同批全部 ordinary items、完整 candidate artifact、score store/identity、runtime policy、analyzer 与 rescope report;缓存 audit/release 不能掩盖物理漂移。当前三个学生运行时开关均为 false(2026-07-19 授权接线及 2026-07-24 试点执行均未触碰这三个开关);ordinary 为 `foundationReady=true`,`liveArtifactVerifierReady=true`,`r3AcceptanceReady=true`,`freshBlindEvidence.ready=true`,`authorizationReady=true`,`causalEnergyStatus=excluded-review-only`。`npm run western:project-gate` 按设计非零退出:ordinary 轨 blocking 归零;M3+ v2 五区证据完整(14/14、gold join `12/12`/`8/8`)且 `authorizationReady=true`,仅剩结构性独立的 `m3plus-student-gate-closed`;M4 automatic adoption 仍未达标,`projectReleaseReady` 因此仍为 false。当前合同下首个受控 review-only 批已实际执行并安全完成；`readyToStartControlledPilot=true` 只表示仍可启动后续独立批。
...
- At that historical detector checkpoint, `npm run western:m3plus-monitored-pilot-audit` returned `readyForMonitoredPilot=false` and the detector-scoped release field was false. This remains failure evidence for the retired detector route. It no longer describes the current four-zone v2 route: the six declared-only protected units, independent per-note intonation gold joins, gold-free runtime foundation, physical runtime audit, and independent authorization were all completed by 2026-07-19. One current-contract review-only pilot batch completed safely on 2026-07-24; the default student runtime remains off.
...
**V2-release 剩余缺口(2026-07-24 更新):** 旧 RF 的 4% 全曲 coverage、first-measure 结果、五批 pilot 和 `r2-08` 均只作历史证据。① live artifact verifier(重读/重算物理来源+伪造报告拒绝)已实现;② `r3-02/r3-03` 冷/热缓存、identity/provenance、全 artifact 接受性验证已通过并视为已消费;③ `ordinary-dynamic-shadow-full-score-fresh-blind-v1` 已实现并实跑,新演奏者对原 8 首曲目的完整录音三层分级证据全绿(见 21b),但沿用旧曲目、非新曲目,只够受控试点门槛;④ ordinary 与 M3+ 独立授权均已完成，2026-07-24 当前合同下首个 review-only 批也已实际执行并 `completed_safe`。该批使用已查看录音，只证明运行和复核链，不具发布级 fresh 资格。若要满足更严格的发布级 fresh-blind，仍需 Round 6 反向配平的全新录音、完整位置真值和一次性 untouched 评测；这是当前实质证据缺口，不再是执行器、物理审计、授权或“从未运行”的缺口。公开合成扰动的 clean precision/coverage=`97.91%/36.00%` 是研究正结果;因果能量否决因跨域不通用无法冻结,已按 2026-07-18 负责人决定正式排除(不再作为门槛,状态锁定 `excluded-review-only`)。三个学生端运行时开关(`ordinaryUploadAutoFeedbackReady`/`m3plusAutoFeedbackReady`/`m4OmrAutoScoreReady`)仍为 `false`,开放学生端还需要全新真闸通过及另一次独立、显式的负责人授权。

**本地安全清理:** `npm run western:cleanup` 只预览;`npm run western:cleanup:apply` 仅删除旧 model-bakeoff/Oemer/HOMR/Clarity 隔离环境、废弃第三方源码、M4 smoke 调试目录、可再下载的空缓存、`node_modules/.vite`、`dist/` 和源码树 Python 缓存。本轮被拒绝的 Clarity 候选已按实验裁决单独删除,不会把模型权重写成长期清理规则。正式 benchmark MusicXML/JSON/CSV、适配数据集/训练报告/冻结照片评测以及 `clarity-training-audit`、`clarity-train-source-audit`、`clarity-pretrained` 会保留;脚本拒绝工作区外路径和 `paper/` 目标,不删除正式数据、生产模型、音频、教师复核、private intake 或论文;执行后必须重跑 `npm run build`。
...
进一步的逐页序列归因表明，24 个 onset 失败页只有 `10/24` 可归入常量偏移、分段常量台阶或插删后的单调漂移，`14/24` 为随机型；22 个 measure 失败页只有 `7/22` 可由弱起、反复或行内拍号模式解释。每页标准差、MAD、步数、首个偏移小节和时值配平结果均已冻结在 `clean-failure-modes/structure-attribution.json`。因此当前路线不得把少数后处理规则写成主要解法，应先提升通用节奏/小节重建能力。
...
2026-07-21 退化差距补审:ex09/ex10 相对冻结 synthetic-test 12 张均出现超出合成最大值的色度高频 RMS 与局部墨迹密度，而没有一致的 blur、过曝或照明幅度越界。下一像素增强冻结为 `chromatic-screen-moire` 的 future train/calibration 候选，现有 synthetic-test 与 fresh-blind 均不回写；高密度版面是源内容多样性问题，后续应增加公开专业单谱表密集版式，不得用像素滤镜替代。
