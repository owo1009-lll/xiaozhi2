...
> **Fresh blind 入场(2026-07-10):** 现有 12 条录音全部已经进入历史标签/复核,不能再用于最终盲审。运行 `npm run western:fresh-blind-intake-init` 创建私有模板,填入全新录音、已审 clean MusicXML/MXL 和对应谱面图片/PDF 后,必须先通过 `npm run western:fresh-blind-intake-status`。该闸门按录音 ID、音频哈希、曲目 ID和谱面内容防重复,并检查音频解码、第一小节音符、谱面批准/显示文件/授权。未返回 `readyForMachinePrecheck=true` 前不得写入受控 intake 或生成教师包。
