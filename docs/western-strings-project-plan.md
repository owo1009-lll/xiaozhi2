...
- **第二轮真实对齐复验(2026-07-15):** 新增 `npm run western:round2-m3plus-eval`,用 Basic Pitch 序列 DTW 替代旧线性时间窗。项目负责人已确认 `r2-06` 实际演奏了谱面标出的 6 个颤音,其余 16 个长音使用揉弦;确认记录保存在 `docs/western-strings-round2-m3plus-human-gold.json`。实测滑音 7/12(58.3%)、颤音 0/6、揉弦 1/16(6.3%)、双音两声部完整识别 19/24(79.2%),四项均未达 90%。因此旧 `releaseReadyModes` 只能解释为 first-measure 安全子集证据,不能推广到第二轮整段真实录音。当前无足够真实负例,不得靠降阈值凑 recall;所有模式继续 fail-closed。解析器现已保留 MusicXML `harmonic-sounding-pitch/base-pitch/touching-pitch` 角色,仅明确的 sounding pitch 可进入自动音准评分;未限定泛音仍退复核。第二轮仍缺独立装饰音和泛音真实样本。
