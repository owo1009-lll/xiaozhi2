...
16. ✅ 2026-07-09 项目级状态命令已接入:`npm run western:project-status` 同时汇总普通上传候选 gate、M3+ pitch-behavior 复核状态与 M4 OMR benchmark 状态,输出 `data/experiments/western-strings-project-status.json`。当前结果应显示普通上传 `ordinaryUploadAutoFeedbackReady=false`、M3+ `m3plusAutoFeedbackReady=false`、M4 `m4OmrAutoScoreReady=false`;下一步优先级为先标普通上传 blind batch,再标 M3+ 48 条 pitch-behavior 包,然后为 M4 准备独立 gold score 以消除 self-comparison。该命令只读,不得替代 release gate。已新增发布阻断命令 `npm run western:project-gate`;它复用同一份状态逻辑,默认要求 ordinary/m3plus/m4 三条轨道全 ready,当前应以非零退出阻断发布,输出 `data/experiments/western-strings-project-gate.json`。
