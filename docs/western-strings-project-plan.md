...
16. ✅ 2026-07-09/10 项目级状态命令已接入并随当前证据更新:`npm run western:project-status` 同时汇总普通上传候选 gate、confidence pilot + fresh validation eval/release audit、M3+ pitch-behavior 复核/模式评估状态与 M4 OMR benchmark 状态,输出 `data/experiments/western-strings-project-status.json`。当前结果应显示普通上传 `ordinaryUploadAutoFeedbackReady=false`、M3+ `m3plusAutoFeedbackReady=false`、M4 `m4OmrAutoScoreReady=false`。普通上传 confidence validation fresh blind batch 已通过,`npm run western:ordinary-monitored-pilot-smoke` 已验证显式 release flag 下会调用冻结 RF scorer;`npm run western:ordinary-auto-pass-precision-review-pack` 也会先复用已知标签自检,当前结果为 `selfCheckedAutoPassCandidateCount=3`,`knownUsableAutoPassCandidateCount=3`,`knownWrongAutoPassCandidateCount=0`,`unknownReviewCandidateCount=0`,所以这批不需要教师复核。runtime 仍因 `ordinary-auto-gate-disabled-by-default` 保持 fail-closed。M3+ 已有 first-measure slide/trill 离线 release-ready 模式,所以项目状态仍以 `m3plus-runtime-disabled-by-default` 阻断 M3+ 自动反馈。当前项目优先级为:普通上传若要进入受控 pilot,先跑 smoke + precision precheck + `western:ordinary-monitored-pilot-plan`,且该 plan 会在预检缺失、已知错误或未知待复核时 fail-closed;若要推进 M3+,设计 first-measure + trusted-recording + slide/trill monitored pilot;然后为 M4 准备独立 gold score 以消除 self-comparison。项目状态命令只读,不得替代 release gate。已新增发布阻断命令 `npm run western:project-gate`;它复用同一份状态逻辑,默认要求 ordinary/m3plus/m4 三条轨道全 ready,当前应以非零退出阻断发布,输出 `data/experiments/western-strings-project-gate.json`。`npm run test:western-project-gate` 覆盖项目级 fail-closed、confidence validation 状态、显式 release-flag 阻断、M3+ 非 control 模式未达 release 以及 M4 自比样本不被当作独立 gold。
17. ✅ 2026-07-09/10 confidence validation fresh blind batch 已完成并独立评估通过:从下载的 `controlled-candidate-review.completed (2).csv` 复制到 `data/experiments/western-strings-m3/confidence-validation-review/controlled-candidate-review.completed.csv`,运行 `npm run western:controlled-candidate-confidence-validation-eval`。结果为 reviewed/scored=30、usable=27、wrong=3、uncertain=0,冻结模型 RF threshold=0.7 在这批预筛样本上 precision=0.90,满足当前 validation floor。`western:project-status` 现在应显示 `confidencePilot.needsBlindValidation=false`、`validationEval.blindValidationPassed=true`,但 runtime 仍保持 `ordinaryUploadAutoFeedbackReady=false`,阻塞原因包含 `ordinary-auto-gate-disabled-by-default`。runtime gate 已通过 `models/western-strings/ordinary-upload-confidence-rf-v1/release.json`、`scripts/experiments/score_western_controlled_candidate_confidence.py` 与 npm 入口 `western:controlled-candidate-confidence-score` 接线;`npm run western:ordinary-monitored-pilot-smoke` 证明显式设置 `WESTERN_STRINGS_ENABLE_ORDINARY_AUTO_GATE=1` 时会调用冻结 scorer 并写出 `confidenceProbability`,同时默认项目状态仍 fail-closed。`npm run western:controlled-candidate-confidence-release-audit` 现在以 candidate-evidence runtime policy 汇总 fresh validation 与 threshold-pool 证据;`npm run western:ordinary-auto-pass-precision-review-pack` 在进入 pilot plan 前先复用已知标签自检,当前没有未知待复核或已知错误 auto-pass 行。默认不开学生端,只有 smoke、precision precheck 和 `western:ordinary-monitored-pilot-plan` 均通过后,才可在单独受控进程里临时设置 release flag。
