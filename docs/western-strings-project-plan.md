...
- **当前执行状态(2026-07-09):** 已新增 eval-only 清点命令 `npm run western:m3plus-pitch-modes`。全量跑通 12 条真实/准真实录音、2588 个谱面音符,输出 `data/experiments/western-strings-m3plus/m3plus-pitch-mode-inventory.csv` 与 `m3plus-pitch-mode-summary.json`;其中 1269 个音符被标为需关注的 pitch-behavior 候选(以 `slide-like`、`variable-f0` 为主)。已新增 `npm run western:m3plus-review-pack`,从 inventory 抽样生成本地人工复核包 `data/experiments/western-strings-m3plus/pitch-mode-review-pack/index.html`:共 48 条,`slide-like` / `trill-like` / `double-stop-candidate` / `ornament-candidate` / `stable` / `variable-f0` 各 8 条,并附本地短 WAV 与对应五线谱图片(`score-images/`,按 piece/page/measure/note 定位)。复核页已改为正常中文说明,提供单条"匹配且音准正确/不确定/不匹配"快捷按钮,也提供"未标全部设为匹配且正确/不确定"批量按钮;批量按钮只填未标项,不得替代听辨。当前 completed CSV 已导入,`npm run western:m3plus-review-status` 实测 `m3plusModeEvalReady=true`:48 reviewed / 43 scored,每类 reviewed/scored 缺口均为 0。已新增并运行 `npm run western:m3plus-mode-eval`:结果为 `m3plusModeReleaseReady=false`,`releaseReadyModes=[]`,`controlReadyModes=["stable"]`;说明标签足够做评估,但没有任何非 control pitch-behavior 模式达到减少复核的证据。学生端 M3+ 仍全部 `review_required`;若要继续,必须补更多真实 slide/trill/ornament/double-stop/variable-f0 样本后重跑 per-mode precision/unsafe 评估。
...
15. ✅ 2026-07-09 置信模型 pilot 已接入:`npm run western:controlled-candidate-confidence-pilot` 读取累计 60 条 `usable/wrong` 标签,只做 eval-only 训练/验证,不接 runtime gate。主口径使用 deployable 特征(不使用 recordingId / recordingScenario 等会过拟合的元数据)并按 recordingId 留一验证。严格主口径当前推荐 RF threshold=0.7(selected=32,precision=0.9375,coverage=0.5333)。说明复核数据已经从"继续凑样本"进入"训练置信模型"阶段。但这仍是 pilot positive,不是 release:下一步必须用 fresh blind batch 复核来验证同一模型/阈值,通过前普通上传仍保持 `review_required`。验证批次生成命令:`npm run western:controlled-candidate-confidence-validation-export` 先输出 `candidate-confidence-validation-selection.{json,csv}`,再运行 `npm run western:controlled-candidate-confidence-validation-review-pack` 生成 `data/experiments/western-strings-m3/confidence-validation-review/index.html` 供人工盲标。盲标完成后运行 `npm run western:controlled-candidate-confidence-validation-eval` 独立评估同一冻结模型/阈值;该命令不把 fresh CSV 合并进训练 labels,只写 `confidence-validation-eval.json` 和逐行评估 CSV。
16. ✅ 2026-07-09 项目级状态命令已接入并随当前证据更新:`npm run western:project-status` 同时汇总普通上传候选 gate、confidence pilot + fresh validation eval/release audit、M3+ pitch-behavior 复核/模式评估状态与 M4 OMR benchmark 状态,输出 `data/experiments/western-strings-project-status.json`。当前结果应显示普通上传 `ordinaryUploadAutoFeedbackReady=false`、M3+ `m3plusAutoFeedbackReady=false`、M4 `m4OmrAutoScoreReady=false`。普通上传 confidence validation fresh blind batch 已通过,且 runtime smoke 已验证显式 release flag 下会调用冻结 RF scorer;但 runtime 仍因 `ordinary-auto-gate-disabled-by-default` 与 `ordinary-confidence-full-threshold-pool-precision-unmeasured` 保持 fail-closed。M3+ 48 条 pitch-behavior 人工复核已导入且 label gate 通过,但 per-mode eval 仅 `stable` control ready,没有非 control release-ready 模式,所以项目状态仍以 `m3plus-no-mode-specific-release-ready` 阻断 M3+ 自动反馈。当前项目优先级为:普通上传若要进入受控 pilot,先分层复核完整阈值池;若要推进 M3+,补更多真实模式样本后重跑 `western:m3plus-mode-eval`;然后为 M4 准备独立 gold score 以消除 self-comparison。项目状态命令只读,不得替代 release gate。已新增发布阻断命令 `npm run western:project-gate`;它复用同一份状态逻辑,默认要求 ordinary/m3plus/m4 三条轨道全 ready,当前应以非零退出阻断发布,输出 `data/experiments/western-strings-project-gate.json`。`npm run test:western-project-gate` 覆盖项目级 fail-closed、confidence validation 状态、显式 release-flag 阻断、M3+ 非 control 模式未达 release 以及 M4 自比样本不被当作独立 gold。
17. ✅ 2026-07-09 confidence validation fresh blind batch 已完成并独立评估通过:从下载的 `controlled-candidate-review.completed (2).csv` 复制到 `data/experiments/western-strings-m3/confidence-validation-review/controlled-candidate-review.completed.csv`,运行 `npm run western:controlled-candidate-confidence-validation-eval`。结果为 reviewed/scored=30、usable=27、wrong=3、uncertain=0,冻结模型 RF threshold=0.7 在这批预筛样本上 precision=0.90,满足当前 validation floor。`western:project-status` 现在应显示 `confidencePilot.needsBlindValidation=false`、`validationEval.blindValidationPassed=true`,但 runtime 仍保持 `ordinaryUploadAutoFeedbackReady=false`,阻塞原因包含 `ordinary-auto-gate-disabled-by-default`。runtime gate 已通过 `models/western-strings/ordinary-upload-confidence-rf-v1/release.json`、`scripts/experiments/score_western_controlled_candidate_confidence.py` 与 npm 入口 `western:controlled-candidate-confidence-score` 接线;新增 runtime smoke 证明显式设置 `WESTERN_STRINGS_ENABLE_ORDINARY_AUTO_GATE=1` 时会调用冻结 scorer 并写出 `confidenceProbability`。新增 `npm run western:controlled-candidate-confidence-release-audit` 说明 coverage 语义:validation export 先从 2528 个候选中筛出 2291 个阈值以上候选,再抽样 30 条人工复核;因此 validation 的 coverage=1.0 不是全量普通上传覆盖率,完整阈值池 precision 仍未测。默认不开学生端,只有完成完整阈值池分层复核后才可考虑受控 pilot。
...
22. ⏳ 2026-07-09 M3+ 第二轮补强包已生成:`npm run western:m3plus-review-pack-round2` 会从 inventory 中排除第一轮已复核 48 条,生成 `data/experiments/western-strings-m3plus/pitch-mode-review-pack-round2/index.html`。本轮共 36 条,其中 variable-f0/slide-like/trill-like/double-stop-candidate 各 8 条,ornament-candidate 仅 4 条可用候选。标完后用带 `--source`/`--reviews` 的 `western:m3plus-review-import` 导入同一个累计 labels CSV,再跑 `npm run western:m3plus-mode-eval`;未通过前 M3+ 仍全部 review-only。
