...
16. ✅ 2026-07-09 项目级状态命令已接入并随当前证据更新:`npm run western:project-status` 同时汇总普通上传候选 gate、confidence pilot + fresh validation eval、M3+ pitch-behavior 复核/模式评估状态与 M4 OMR benchmark 状态,输出 `data/experiments/western-strings-project-status.json`。当前结果应显示普通上传 `ordinaryUploadAutoFeedbackReady=false`、M3+ `m3plusAutoFeedbackReady=false`、M4 `m4OmrAutoScoreReady=false`。普通上传 confidence validation fresh blind batch 已通过,但 runtime 仍因 `ordinary-auto-gate-disabled-by-default` 保持 fail-closed;`WESTERN_STRINGS_ENABLE_ORDINARY_AUTO_GATE=1` 仅允许在受控 pilot 进程中使用,不得默认长开。M3+ 48 条 pitch-behavior 人工复核已导入且 label gate 通过,但 per-mode eval 仅 `stable` control ready,没有非 control release-ready 模式,所以项目状态仍以 `m3plus-no-mode-specific-release-ready` 阻断 M3+ 自动反馈。当前项目优先级为:先核实普通上传 runtime 特征提取与冻结 RF scorer 的一致性,并排查 pilot coverage=0.5333 / precision=0.9375 vs fresh validation coverage=1.0 / precision=0.90 的操作点漂移;若要推进 M3+,补更多真实模式样本后重跑 `western:m3plus-mode-eval`;然后为 M4 准备独立 gold score 以消除 self-comparison。项目状态命令只读,不得替代 release gate。已新增发布阻断命令 `npm run western:project-gate`;它复用同一份状态逻辑,默认要求 ordinary/m3plus/m4 三条轨道全 ready,当前应以非零退出阻断发布,输出 `data/experiments/western-strings-project-gate.json`。`npm run test:western-project-gate` 覆盖项目级 fail-closed、confidence validation 状态、显式 release-flag 阻断、M3+ 非 control 模式未达 release 以及 M4 自比样本不被当作独立 gold。
17. ✅ 2026-07-09 confidence validation fresh blind batch 已完成并独立评估通过:从下载的 `controlled-candidate-review.completed (2).csv` 复制到 `data/experiments/western-strings-m3/confidence-validation-review/controlled-candidate-review.completed.csv`,运行 `npm run western:controlled-candidate-confidence-validation-eval`。结果为 reviewed/scored=30、usable=27、wrong=3、uncertain=0,冻结模型 RF threshold=0.7 选中 30/30,precision=0.90、coverage=1.0,满足当前 validation floor。`western:project-status` 现在应显示 `confidencePilot.needsBlindValidation=false`、`validationEval.blindValidationPassed=true`,但 runtime 仍保持 `ordinaryUploadAutoFeedbackReady=false`,阻塞原因转为 `ordinary-auto-gate-disabled-by-default`。runtime gate 已通过 `models/western-strings/ordinary-upload-confidence-rf-v1/release.json`、`scripts/experiments/score_western_controlled_candidate_confidence.py` 与 npm 入口 `western:controlled-candidate-confidence-score` 接线;默认不开学生端,只有显式设置 `WESTERN_STRINGS_ENABLE_ORDINARY_AUTO_GATE=1` 才允许候选级 auto-pass,否则继续 fail-closed。
