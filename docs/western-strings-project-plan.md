...
- **当前执行状态(2026-07-09):** 已新增 eval-only 清点命令 `npm run western:m3plus-pitch-modes`。全量跑通 12 条真实/准真实录音、2588 个谱面音符,输出 `data/experiments/western-strings-m3plus/m3plus-pitch-mode-inventory.csv` 与 `m3plus-pitch-mode-summary.json`;其中 1269 个音符被标为需关注的 pitch-behavior 候选(以 `slide-like`、`variable-f0` 为主)。已新增 `npm run western:m3plus-review-pack`,从 inventory 抽样生成本地人工复核包 `data/experiments/western-strings-m3plus/pitch-mode-review-pack/index.html`:共 48 条,`slide-like` / `trill-like` / `double-stop-candidate` / `ornament-candidate` / `stable` / `variable-f0` 各 8 条,并附本地短 WAV。标完网页下载 `m3plus-pitch-mode-review.completed.csv` 后,运行 `npm run western:m3plus-review-import` 合并标签,再用 `npm run western:m3plus-review-status` 查看每类 reviewed/scored 缺口。该结果只用于抽样复核和后续标注规划,**不验证 M3+ precision,不改变学生端 gate,所有候选仍为 `review_required`**。下一闸门必须有人类/gold 标注证明具体模式下音准 precision≥90%、unsafe=0 后,才能减少复核。
...
16. ✅ 2026-07-09 项目级状态命令已接入:`npm run western:project-status` 同时汇总普通上传候选 gate、confidence pilot + fresh validation eval、M3+ pitch-behavior 复核状态与 M4 OMR benchmark 状态,输出 `data/experiments/western-strings-project-status.json`。当前结果应显示普通上传 `ordinaryUploadAutoFeedbackReady=false`、M3+ `m3plusAutoFeedbackReady=false`、M4 `m4OmrAutoScoreReady=false`;普通上传还会显示 `confidencePilot.releaseCandidateFound=true` 与 `needsBlindValidation=true`,并把 next action 指向 `data/experiments/western-strings-m3/confidence-validation-review/index.html`,但 runtime 仍因 `candidate-confidence-pilot-needs-blind-validation` 保持 fail-closed。下一步优先级为先标完 confidence validation fresh blind batch,运行 `western:controlled-candidate-confidence-validation-eval` 查看 `blindValidationPassed`,再标 M3+ 48 条 pitch-behavior 包,然后为 M4 准备独立 gold score 以消除 self-comparison。即使 blind validation 通过,也只能作为下一阶段 runtime gate wiring 的证据,不得由状态命令直接开启学生端。该命令只读,不得替代 release gate。已新增发布阻断命令 `npm run western:project-gate`;它复用同一份状态逻辑,默认要求 ordinary/m3plus/m4 三条轨道全 ready,当前应以非零退出阻断发布,输出 `data/experiments/western-strings-project-gate.json`。`npm run test:western-project-gate` 覆盖项目级 fail-closed、confidence pilot blind-validation 阻断与 M4 自比样本不被当作独立 gold。
