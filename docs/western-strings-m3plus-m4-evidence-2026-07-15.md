...
为判断弱音漏放能否只靠调参解决，又在开发演奏者上联合 sweep 了 `relativeIoiDeviationRatio`、事件绝对置信度、相对邻音置信度和事件时长，共 `2016` 个运行时可见操作点。没有任何点同时满足“开发集全部四类错误零漏放 + clean coverage >=20%”。最佳零漏放点 clean precision=`99.27%`、coverage=`16.57%`；冻结后在未见演奏者上 precision=`97.88%`、coverage=`15.95%`，弱音仍漏放 `2/48`。因此弱音缺口不能靠这些阈值继续调平；若要求零漏放，覆盖会低于现有 20% 产品地板，保持 review-only。
