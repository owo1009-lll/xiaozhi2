...
- Clarity 监督适配的非人工前置已接入:`western:m4-clarity-adaptation-data-probe` 可生成去重的 staff/token 训练对;`western:m4-clarity-adaptation-split` 按 BWV 作品拆成 `21/4/7`,冻结真实照片 gold 不入训练;`western:m4-clarity-training-step` 已在 RTX 5060 上以 bf16+DoRA 完成一次有限 loss/gradient 的训练步,峰值 reserved 显存约 `1.08 GiB`。该结果仅为训练可行性,没有准确率结论、没有生产 checkpoint。受限训练后仍须由 validation、synthetic-test 和冻结真实照片共同通过完整 pitch/onset/measure 闸门,否则保持 eval-only。
...
- 2026-07-09/10 更新:confidence validation fresh blind batch 已完成并独立评估通过。`npm run western:controlled-candidate-confidence-validation-eval` 在 30 条 fresh completed CSV 上得到 usable=27 / wrong=3 / uncertain=0,冻结 RF threshold=0.7 在这批预筛样本上 precision=0.90。该结果满足当前 validation floor;runtime confidence gate 已接线到 `models/western-strings/ordinary-upload-confidence-rf-v1/release.json` 与 `scripts/experiments/score_western_controlled_candidate_confidence.py`,且 `npm run western:ordinary-monitored-pilot-audit` 已验证真实受控队列 precheck、显式 flag smoke 和 pilot plan 均通过。但默认仍 fail-closed:`western:project-status` 继续显示 `ordinaryUploadAutoFeedbackReady=false`。`npm run western:controlled-candidate-confidence-release-audit` 现在以 candidate-evidence runtime policy 汇总 fresh validation 与 threshold-pool 证据;`npm run western:ordinary-auto-pass-precision-review-pack` 已复用历史已知标签做自测,当前 self-checked auto-pass 3 条均为 known usable,0 条 known wrong,0 条 unknown review rows,所以这批不要求教师复核。M3+ first-measure slide/trill 已有离线 release evidence;M4 也已转为 `human-approved-unchanged-draft` 口径并清除人工 gold 阻断。`npm run western:release-review` 现已成为发布前总控命令,当前输出 `readyForControlledPilot=true`,`readyForDefaultStudentRelease=false`,`teacherReviewNeeded=false`,`runtimeFailClosed=true`。当前裁定是保持默认关闭:只有在单独受控 pilot 进程中才可显式设置 `WESTERN_STRINGS_ENABLE_ORDINARY_AUTO_GATE=1`;无该环境变量、release/validation/precheck 缺失或 scorer 失败时均回退 review-only。
...
- 2026-07-09/10 更新:P1.1 context-feature 重校准与 fresh validation 已完成。当前冻结 release artifact threshold=0.8;release audit 报告 pilot precision=0.942857、validation precision=1、runtime-selected threshold-pool precision=1(12 selected / 53 reviewed)。`western:ordinary-monitored-pilot-audit` 已通过且不需要继续教师复核,但默认学生端仍 fail-closed;只有负责人批准后的单独受控 pilot 可临时开启。
...
- `npm run western:controlled-pilot-record-decision -- --decision defer --by <负责人>` 记录暂缓/不启动,继续 review-only。
...
- 现有 12 条录音全部已进入历史标签/复核,不能再作 fresh blind evidence。全新录音、已审 clean MusicXML/MXL 与谱面图片/PDF 就位后,使用 `npm run western:fresh-blind-intake-stage` 原子化登记,不再手改 JSON;该命令先在临时清单执行 ID/内容哈希去重、音频解码、第一小节可分析性、谱面批准和授权检查,全过才替换正式 intake。随后必须通过 `npm run western:fresh-blind-intake-status`,才允许进入普通机器预检。未通过不得生成专业复核包。
