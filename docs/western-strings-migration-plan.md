...
- 已接入 `npm run western:m3plus-review-import` 与 `npm run western:m3plus-review-status`:标完网页下载 `m3plus-pitch-mode-review.completed.csv` 后导入,状态命令会报告每类 reviewed/scored 缺口。第一轮 48 条与第二轮 36 条补强样本已累计导入,实测 `m3plusModeEvalReady=true`:84 reviewed / 60 scored,每类 reviewed/scored 缺口均为 0。
- 已接入并运行 `npm run western:m3plus-mode-eval`:结果为 `m3plusModeReleaseReady=false`,`releaseReadyModes=[]`,`controlReadyModes=["stable"]`,`blockingReasons=["m3plus-no-mode-specific-release-ready"]`。也就是说 stable 对照样本可作为 control,但没有任何非 control pitch-behavior 模式具备减少复核的 release 证据。累计复核还显示 60 match / 19 mismatch / 5 uncertain-or-other,谱面-录音定位质量本身是当前 blocker。
...
- 2026-07-09 更新:M3+ 第二轮补强包已导入并评估。第一轮 48 条 + 第二轮 36 条累计为 84 reviewed / 60 scored;`npm run western:m3plus-mode-eval` 仍显示 `releaseReadyModes=[]`,仅 `stable` control ready。非 control 模式继续 review-only,且 19 mismatch + 5 uncertain-or-other 说明谱面-录音定位/候选质量需要先修,不是继续复用同一包补标。
