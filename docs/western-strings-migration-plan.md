...
- 已接入 `npm run western:m3plus-review-import` 与 `npm run western:m3plus-review-status`:标完网页下载 `m3plus-pitch-mode-review.completed.csv` 后导入,状态命令会报告每类 reviewed/scored 缺口。当前 completed CSV 已导入,实测 `m3plusModeEvalReady=true`:48 reviewed / 43 scored,每类 reviewed/scored 缺口均为 0。
- 已接入并运行 `npm run western:m3plus-mode-eval`:结果为 `m3plusModeReleaseReady=false`,`releaseReadyModes=[]`,`controlReadyModes=["stable"]`,`blockingReasons=["m3plus-no-mode-specific-release-ready"]`。也就是说 stable 对照样本可作为 control,但没有任何非 control pitch-behavior 模式具备减少复核的 release 证据。
- 当前 student gate 仍为 `studentGateReady=false`。M3+ 标签状态已齐备,但 per-mode precision/unsafe 评估未放行任何非 control 模式;未达音准 precision≥90%、unsafe=0 的模式继续 `review_required`。
...
- 2026-07-09 更新:confidence validation fresh blind batch 已完成并独立评估通过。`npm run western:controlled-candidate-confidence-validation-eval` 在 30 条 fresh completed CSV 上得到 usable=27 / wrong=3 / uncertain=0,冻结 RF threshold=0.7 在这批预筛样本上 precision=0.90。该结果满足当前 validation floor;runtime confidence gate 已接线到 `models/western-strings/ordinary-upload-confidence-rf-v1/release.json` 与 `scripts/experiments/score_western_controlled_candidate_confidence.py`,且 runtime smoke 已验证显式 flag 下会调用冻结 scorer 并写出 confidence probabilities。但默认仍 fail-closed:`western:project-status` 继续显示 `ordinaryUploadAutoFeedbackReady=false`。新增 `npm run western:controlled-candidate-confidence-release-audit` 解释 coverage 语义:validation export 先从 2528 个候选中筛出 2291 个阈值以上候选,再抽样 30 条人工复核;因此 validation 的 coverage=1.0 不是全量普通上传覆盖率,完整阈值池 precision 仍未测。当前裁定是保持默认关闭:只有受控 pilot 进程可显式设置 `WESTERN_STRINGS_ENABLE_ORDINARY_AUTO_GATE=1`;无该环境变量、release/validation 缺失或 scorer 失败时均回退 review-only。M3+ 48 条人工复核已完成并导入,`m3plusModeEvalReady=true`;per-mode eval 仅 stable control ready,没有非 control release-ready 模式。当前项目级下一步排序为:若要推进普通上传,先对完整阈值池做分层复核;若要推进 M3+,补更多真实模式样本后重跑 `western:m3plus-mode-eval`;再为 M4 准备独立 gold score。
...
- 2026-07-09 更新:完整阈值池分层复核已完成但未通过。60 行复核为 usable=23 / wrong=36 / uncertain=1;`npm run western:controlled-candidate-confidence-stratified-eval` 得到 selected precision=0.5556、coverage=0.6102,远低于 0.90 release floor。`npm run western:controlled-candidate-confidence-release-audit` 现报告 `ordinary-confidence-threshold-pool-precision-too-low`。当前 RF gate 不得进入受控 pilot;普通上传自动诊断继续默认关闭。下一步改为分析 wrong selected 样本并重校准 confidence 模型/特征,而不是继续开启该 release candidate。
