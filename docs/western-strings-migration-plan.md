...
- 已接入 `npm run western:m3plus-review-pack`,生成 `data/experiments/western-strings-m3plus/pitch-mode-review-pack/index.html`、待填 CSV/JSON/guide 和 48 个本地短 WAV。当前抽样为 6 类各 8 条:`slide-like`、`trill-like`、`double-stop-candidate`、`ornament-candidate`、`stable`、`variable-f0`。复核页已改为正常中文说明,并加入单条/批量快捷按钮;批量按钮只填未标项,不得替代听辨。
...
- 当前 gate 仍为 `studentGateReady=false`。状态命令当前阻塞原因为 `m3plus-review-labels-missing`、`m3plus-review-completed-csv-missing`、`m3plus-review-reviewed-per-mode-too-low`、`m3plus-review-scored-per-mode-too-low`;生成包本身的 gate reason 是 `human-label-collection-only`。这些候选只用于抽样复核和 M3+ 标注规划,不证明 precision,不改变学生端反馈。
