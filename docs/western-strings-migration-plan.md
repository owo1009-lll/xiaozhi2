...
- 2026-07-09 更新:confidence validation fresh blind batch 已完成并独立评估通过。`npm run western:controlled-candidate-confidence-validation-eval` 在 30 条 fresh completed CSV 上得到 usable=27 / wrong=3 / uncertain=0,冻结 RF threshold=0.7 选中 30 条,precision=0.90、coverage=1.0。该结果满足当前 validation floor;runtime confidence gate 已接线到 `models/western-strings/ordinary-upload-confidence-rf-v1/release.json` 与 `scripts/experiments/score_western_controlled_candidate_confidence.py`,但默认仍 fail-closed:`western:project-status` 继续显示 `ordinaryUploadAutoFeedbackReady=false`,阻塞原因为 `ordinary-auto-gate-disabled-by-default`。当前裁定是保持默认关闭:只有受控 pilot 进程可显式设置 `WESTERN_STRINGS_ENABLE_ORDINARY_AUTO_GATE=1`;无该环境变量、release/validation 缺失或 scorer 失败时均回退 review-only。M3+ 48 条人工复核已完成并导入,`m3plusModeEvalReady=true`;per-mode eval 仅 stable control ready,没有非 control release-ready 模式。当前项目级下一步排序为:先核实普通上传 runtime 特征提取与冻结 RF scorer 口径一致,并排查 pilot coverage=0.5333 / precision=0.9375 vs fresh validation coverage=1.0 / precision=0.90 的操作点漂移;若要推进 M3+,补更多真实模式样本后重跑 `western:m3plus-mode-eval`;再为 M4 准备独立 gold score。
