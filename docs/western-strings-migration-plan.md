...
- 当前 student gate 仍为 `studentGateReady=false`。M3+ 标签状态已不再阻塞项目级状态,但这只表示离线模式评估输入齐备,不证明 precision,不改变学生端反馈。下一步必须基于这些标签做 per-mode precision/unsafe 评估;未达音准 precision≥90%、unsafe=0 的模式继续 `review_required`。
