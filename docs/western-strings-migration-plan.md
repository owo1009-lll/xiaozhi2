...
- 2026-07-09 更新:M3+ 第二轮补强包已生成。第一轮 48 条证明 stable 仅可作 control,非 control 模式无 release-ready 证据;`npm run western:m3plus-review-pack-round2` 现排除已复核样本并导出 36 条非 control 补强样本到 `data/experiments/western-strings-m3plus/pitch-mode-review-pack-round2/index.html`。本轮用于补 variable-f0、slide-like、trill-like、double-stop-candidate,ornament-candidate 因 inventory 仅余 4 条可用候选而不足。导入和 per-mode eval 通过前,M3+ 继续 review-only。
