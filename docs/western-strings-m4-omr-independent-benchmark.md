...
- 5/5 原始 source 照片均成功输出,聚合 pitch P/R=`89.00%/96.17%`,onset-quarter/measure accuracy=`30.73%/79.04%`。它的 recall 明显优于 Audiveris/Oemer,但节奏结构仍远低于 95% 门槛。(此为 07-15 首跑历史值;当前权威口径为 2026-07-17 fresh 复验:P/R=`88.33%/95.78%`、onset-quarter=`30.03%`、measure=`79.04%`,严格 0/5 不变,见本文 2026-07-16/17 补充节。)
...
**边界(三层,勿混为一谈,2026-07-17 反审查修正)**:(1) 离线 v3 管线内,HOMR 过自身 MusicXML-only 结构闸后与 Audiveris 同权仲裁,胜出即产出机器裁决(full/degraded),函数内无人工环节;(2) 受控 batch(`run-western-photo-score-batch.mjs`)仍有人工作流前置——只处理人工标为 `accepted_for_batch` 的提交,并强制 `studentFacing=false`、`autoDiagnosisIssued=false`,产物只进教师复核;(3) 正式 analyzer 主线将 HOMR 登记为 `mainlineExecutable=false`(python-service/analyzer_score_import.py),学生端发布与自动采纳均未开放。"HOMR 已入池"仅指第 (1) 层,不构成"进入实际生产运行"。节奏车道纪律不变(OMR 谱不做节奏硬判);发布位以机读字段为准:`m4OmrAutoScoreReady=false`(学生端运行时)、`m4OmrAutomaticAdoptionReady=false` 与 `m4HomrAutomaticAdoptionReady=false`(严格自动采纳)——本节旧文误写的 `m4OmrAutoFeedbackReady` 不是代码中的真实字段,以本句为准。审计 `pipeline: western-photo-score-v3-homr-pool`,增 `winnerEngine`/`winnerAnnotationStyle`/`enginePool` 字段。

运维/治理警示(2026-07-17 审查补记,同日反审查扩充):(1) HOMR 0.7.0 许可证为 **AGPL-3.0**(venv METADATA 与上游仓库一致),按 MUSC 先例(AGPL 审查完成前不得接生产)需补一份正式许可证审查记录;独立子进程调用未修改发行版是有利因素,但是否构成 arm's-length 独立程序取决于通信机制与数据语义(GNU FAQ),属法律判断,工程材料可半天备齐、法律签核时长不保证。(2) 生产管线 `DEFAULT_HOMR` 指向 gitignored 的 `data/experiments/.../homr-compat-venv`,venv 缺失时池静默退化为纯 Audiveris(机器可用 9/12→3/12);且 `cleanup-western-workspace.ps1` 目标列表(第 40 行)会删除该 venv——清理脚本与部署固化直接冲突,固化前不得在部署机运行清理。(3) 三套 Python 环境互斥:系统音频环境 NumPy 1.26.4(basic-pitch/TF)、analyzer venv 2.2.5(无 basic_pitch)、HOMR venv ≥2.4.2;隔离方式不限 venv(容器/独立服务均可),但不可合并。未显式设置 `ERHU_PYTHON_EXE` 时 `run-python.ps1` 默认选 analyzer venv,受控 batch 会在音频事件阶段(import basic_pitch)即失败,到不了 HOMR——启动 preflight 必须自检解释器。(4) Route B 报告 §5 "Audiveris 主引擎 + oemer 救援"的架构描述相对 v3 已过时:oemer 不在 v3 生产池中,仅保留 eval 基准与坐标 sidecar 角色。(5) 07-17 fresh 复验原始 JSON 与 6 套注入集均在 gitignored `data/` 下,远端只有文档结论;应提交紧凑 manifest(SHA-256+版本+聚合值)冻结权威证据。(6) 总 gate 目前没有机读 `licenseReviewStatus`、HOMR 可执行性、依赖锁或启动自检——文档警示不构成机器防线,待 P0 落实。
