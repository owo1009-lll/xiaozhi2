...
- 节奏通道：字段并非空置，round4 有 246/265 行 relative-IOI 证据；冻结单阈值 0.15 虽抓到 5/6 个 drag/extra，却产生 37 个误报。IOI+时长合取及通用 onset/edit-path 单特征也没有可迁移的联合过门点。随后只保留多信号结构合取：relative-IOI、时值比、事件置信度与冻结 operation-path 必须同位置成立。时值比 `>=1.20` 的软层在合成留出为 15/27、0 FP，在已查看 round4 为 4/6、0 FP，自然正确提示为 5/285=1.75%；复用已有冻结值 `>=1.30` 的高置信层在 calibration/holdout 为 18/27、14/27（均 0 FP），round4 为 4/6、0 FP，自然正确为 0/285。它与已有 2 个严格确诊的回顾性上限是 6/12、0/253，再加 gap 自查的三层复核上限是 10/12、0/253。由于结构合取形成于查看旧证据之后，高置信层也只是 `issue_detected_candidate`，正式确诊仍为 2/12；必须在全新 complete-inventory fresh-blind 上原样验收，禁止再看新包调阈值。
- gap 严格候选：原始 gap 精炼在公开专业长曲有 595 个提示，因此继续保留为自查；新增 recording-level 对齐健康守卫（gap `<=5` 且 rate `<=10%`）后，missing-targeted 严格层在合成 calibration/holdout 均为 10/15、0 strict FP，round4 为 3/3 missing 加 1 个真实 wrong、0/253，自然正确 0/285。两段专业长曲的 gap rate 23.57%/47.65%，整条回退为证据不足，595 个原始提示不升级为指控。与原有 2 个严格确诊和节奏严格层 4 个合并的回顾性严格候选上限为 10/12、0/253；守卫同样是查看旧证据后形成，只能进入 untouched Round 5，正式确诊仍为 2/12。
