...
- 节奏通道：字段并非空置，round4 有 246/265 行 relative-IOI 证据；冻结单阈值 0.15 虽抓到 5/6 个 drag/extra，却产生 37 个误报。IOI+时长合取在合成开发/holdout 都达到 P/R=95.45%/77.78%，但完整 round4 只有 50.00%/66.67%，域迁移失败。通用 onset 峰、同音高门控 onset、未分配事件 edit-path 和内部攻击比也都没有 P>=90%/R>=50% 的单独操作点。随后仅作结构合取：relative-IOI、时值比、事件置信度与冻结 operation-path 必须同位置成立；它在合成留出为 15/27、0 FP，在已查看 round4 为 4/6、0 FP（drag 3/3、extra 1/3），自然正确录音提示负担为 5/285=1.75%。因为规则形成于查看旧证据之后，它只冻结为 Round 5 `self_check_hint` 子闸，不能晋升或自动指控；下一步必须在全新 position-labelled fresh-blind 上原样验收，禁止再看新包调阈值。
