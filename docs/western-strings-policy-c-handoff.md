...
- 节奏通道：字段并非空置，round4 有 246/265 行 relative-IOI 证据；冻结单阈值 0.15 虽抓到 5/6 个 drag/extra，却产生 37 个误报。IOI+时长合取在合成开发/holdout 都达到 P/R=95.45%/77.78%，但完整 round4 只有 50.00%/66.67%，域迁移失败。通用 onset 峰、同音高门控 onset、未分配事件 edit-path 和内部攻击比均已按开发/holdout 协议实测，全部没有 P>=90%/R>=50% 的可用操作点。因此停止追加单阈值或单一手工攻击特征；下一候选必须用定向真实重复起弓/正常换弓/揉弦峰混淆标签训练分段 insert/delete edit-path 模型，并在全新 position-labelled fresh-blind 上验收。
