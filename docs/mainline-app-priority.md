...
- Widened the scanned/generic `Voice` erhu-range fallback for high-confidence, single-staff candidates with moderate chord evidence (`chordRatio <= 0.14`) while keeping the stricter per-line monophonic gate. Re-running the local complex-PDF benchmark now gives `3/3` completed with `erhuRatio = 1.0` for `Liulangzhe Zhi Ge`, `Fusheng`, and `Violeta Suite 07`; `Fusheng` improved from `144/452` erhu notes to `452/452`.
