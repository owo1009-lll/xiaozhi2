...
**现在上线的形态是「受控试点」**:学生能上传练习录音,由你在本机复核后放行反馈;机器自动判分暂时是关的。2026-07-24 已完成一个当前合同下的离线 review-only 批，证明全谱候选与 M3+ 复核链可以实际运行，但它使用的是已查看录音，不能作为学生自动判分的 fresh 发布证据。自动判分必须等待 Round 6 的 12 条反向配平新录音通过一次性 `90% precision / 50% recall / 0 strict-FP` 真闸，并另行取得学生端发布授权；不能再写成“等 round4 6 条录音后直接开开关”。
...
- **自动判分仍是关的**(试点形态)。当前严格确诊召回仍为 `2/12`；需要 Round 6 全新录音真闸通过、ordinary/M3+ 学生闸分别满足并取得显式发布授权后才能讨论开关，不能用本次 review-only 试点代签。
