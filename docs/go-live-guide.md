...
**现在上线的形态是「受控试点」**:学生能上传练习录音,由你在本机复核后放行反馈;机器自动判分暂时是关的。2026-07-24 已完成一个当前合同下的离线 review-only 批，证明全谱候选与 M3+ 复核链可以实际运行，但它使用的是已查看录音，不能作为学生自动判分的 fresh 发布证据。P0–P2 零录音审计已经榨完，Round 6 当前只授权先录 6 条反向配平 calibration；只有它训练出的冻结候选先通过既有真实干净域安全闸，才条件追加 6 条 untouched fresh 并执行一次性 `90% precision / 50% recall / 0 strict-FP` 真闸。Round 6 无作用域的一次性 12 条签署入口已经关闭；Stage B 只能签署 fresh 六条并复用 Stage A 模型，执行证据必须为 `trainingPerformed=false / frozenModelLoaded=true`。数值通过后仍须另行取得学生端发布授权；不能再写成“现在一次录满 12 条”或“等 round4 6 条后直接开开关”。
