...
- **依赖审计归零且不改冻结清单:** 在 `package.json` 既有 semver 范围内更新 lockfile，修复 Multer、`body-parser`/`qs`、Vite、PostCSS、esbuild 与 Babel 链的 7 组审计项；完整 `npm audit` 当前为 0。冻结 `package.json` 的 SHA-256 仍为 `e2a7b694ed61aeb8fdf8713f6d16092dbd175cdebb66e045a7b6ab1fa95284f6`，Round 6 source binding 未变化。
- **公网 P0 边界已收口:** Cloudflare 隧道白名单现在先于 120 MiB JSON parser 执行，public mode 强制 loopback bind；曲谱导入、普通音频和西洋弦乐双文件使用三套最小兼容的内存上传上限，避免单一宽 profile 放大内存占用。公网分析拒绝读取受管缓存外的本机音频、响应不再返回绝对路径/内部 submission；公开提交与记录查询在解析/缓冲前按 Cloudflare 客户 IP 限频并限制并发。小程序提交和查询均由服务端把 `wx.login` 换得的 OpenID 绑定为不可逆学生身份，客户端自报 `studentRef` 不再授权；网页版改用 128 位随机 capability，旧可猜标识仅可在本机历史复核流使用。
- **训练账本完整性已收口到单进程边界:** 每次签署重新读取并哈希受管 `data/` 内的真实音频与 candidate artifact，拒绝缺失、变化、越界和 reparse-point 逃逸；追加前复核整条 revision/hash/录音/谱面身份链，同一 Node 进程内按账本文件串行写入，并拒绝 Windows 文件名碰撞、非法 extra-event 时间/MIDI 和匿名完整签署。8 路并发、早期记录篡改及缺失/变更音频回归均通过；当前真实账本仍为 `recordings=1`、`invalidRecords=[]`。
- **仍需保留的边界:** 若未来启动多个 Node 写进程，JSONL 账本须升级为跨进程文件锁或 SQLite 事务；应用层限流不能替代 Cloudflare 边缘限流；清理脚本遇到删除失败仍会 fail-fast，尚未实现“两阶段预检 + 完整失败 JSON”。这些均不打开学生自动诊断；`ordinaryUploadAutoFeedbackReady=false`、`m3plusAutoFeedbackReady=false`、`m4OmrAutoScoreReady=false` 保持不变。

## 2026-07-28 训练账本首条真实闭环

- 使用已有完整真值的 `round4-r4-06` 跑通“提交 → 接受入批 → 单项 `run-batch` → 载入 40/40 全谱 → 人工完整签署 → 放行反馈”。训练账本现场状态为 `recordings=1`、`signatures=1`、`performers=1`、`implicitCorrect=36`，显式标签为 `wrong_pitch=1 / missing=2 / drag=1`，另有 2 个独立 `extraEvents`；落盘记录通过 candidate artifact、score-note identity、音频和哈希链复验，`invalidRecords=[]`。学生记录已转为 `feedback_released`，但三个学生自动开关仍全为 `false`，账本不接任何发布闸。
- 闭环同时发现并修复真实上传兼容缺口：网页/小程序上传缓存沿用 40 位 SHA-1 身份，而账本原先错误地把 `submission.audioHash` 强制解释为 64 位 SHA-256，导致真实上传必然拒绝入账。账本现在先按提交时算法核验落盘音频身份，再对同一字节现场派生并保存 SHA-256；新增回归覆盖 SHA-1 上传身份，未放宽音频内容校验。新提交默认仍须先执行 `accepted_for_batch` 才会被 `run-batch` 消费，故完整操作流程必须显式包含“接受入批”。
...
- **永久性修复已落地:** ordinary r3 live verifier 与 controlled-batch physical audit 已改为 `referenced-score-payload-and-note-identity-v1`：`scoreStoreArtifactSha256` 继续保留为生成时 provenance，但不再把“无关曲谱新增导致的整个 SQLite 哈希变化”当作证据过期。审计仍现场打开 canonical store；引用 `scoreId` 缺失、引用 score payload 变化、音符数/逐音身份变化、store 不可读或审计期间变化均继续 fail-closed。r3 acceptance 升为 schema/contract v2，live verifier 升为 `western-ordinary-dynamic-shadow-live-artifact-verifier-v2`，组合 live-evidence 合同升为 `western-controlled-pilot-live-evidence-v2`，旧全库绑定合同不能静默沿用。回归测试已同时覆盖“新增无关曲谱继续通过”和“修改引用曲谱立即转红”；现有 59 行物理批在不重跑物理分析的情况下通过新审计，证明以后扩充曲库不再要求整链重做。

## 2026-07-19 当前分支刷新

- **2026-07-27 P3 预注册 package.json 绑定重签(负责人 guanxingzhi 授权):** 为训练账本增加两条 npm script 时改动了 `package.json`,而它是 `docs/evidence/western-strings-p3-minimal-recording-preregistration-20260724.json` 的 13 个 `sourceBindings` 之一,导致 `sourceBindingsCurrent=false`、`recordingSchedule.valid=false`、`stageARecordingAuthorizedNow` 由 true 翻为 false,`test:western-project-gate` 在 stagedProtocol 断言处失败。随后用 `git checkout -- package.json` 回滚是第二个错误:本机 `core.autocrlf=true` 且该文件索引 stat 缓存陈旧(`git status` 误报干净),checkout 覆盖掉了未提交的本地内容;已在全部 196 个历史 `package.json` blob 中搜索,原字节从未提交、不可恢复,因此重签是唯一路径。本次**只**修改 `sourceBindings` 中 `package.json` 条目的 `sha256`(`ebebef0a…89fd6` → `e2a7b694…84f6`),不新增任何顶层字段——语义核心仅排除 `schemaVersion`/`sourceBindings`/`protocolSemanticSha256`,故新增字段会破坏语义哈希。重签后现场复核:13/13 绑定通过、语义哈希 `fe5302fe…07ccb` 记录值与重算值逐位一致(**证明协议语义未被改动**)、8 条 Stage A/B 协议命令逐条完好、`stageARecordingAuthorizedNow=true`、作用域仍为 `stage-a-calibration-six-only`、`stageBFreshRecordingAuthorizedNow=false`、学生三开关未受影响仍全 false、`test:western-project-gate` 退出码 0。**未改动**:`config/western-strings-round6-evaluation-protocol.json`(仅按路径引用,不绑 SHA,无级联)与 `docs/evidence/western-strings-zero-recording-stakeholder-report-20260724.artifact.json`(其中记录的预注册 SHA `fe0c7fdc…` 现已过期,但无任何自动校验消费它,且它是 2026-07-24 的带日期快照,回改等同篡改已发布件)。**遗留损失**:5 个未提交的本地 npm alias 丢失(`build-public-score-library`、`ensure-public-backend`、`watch-public-backend`、`import-mutopia-violin-library`、`test-public-score-library` 对应的别名,原名未知);脚本文件本身完好,直接用 `node`/`powershell` 调用即可。**常备约束**:Round 6 Stage A/B 走完前 `package.json` 视为冻结文件,新脚本一律直接调用,不加 npm alias;必须改动时须先取得负责人署名并按本条同样方式重签。
- **2026-07-27 复核台训练数据账本已落地(不接任何闸):** 按 `docs/western-strings-training-ledger-spec.md` 实现 `src/server/westernStringsTrainingLedger.js`,在 `recordWesternControlledSubmissionReview` 写入复核记录**之后**追加一步:仅当本次复核签署了 `completeErrorInventory=true` 且 `performerId`、`consent=yes` 齐全时,把逐音金标 + 机器特征快照指针以 JSONL 只追加方式写入 gitignored 的 `data/private/western-strings-training-ledger/<recordingId>.jsonl`。账本失败以数据形式返回、**绝不抛进复核流**,未签署的复核行为与改动前逐字节一致。复核台新增逐音四态打标面板与只读端点 `GET /api/strings/controlled-submissions/:submissionId/score-notes`(供标注机器未命中的谱音);该端点不在公网白名单内,隧道流量按默认拒绝返回 403。纪律测试 `node scripts/test-western-strings-training-ledger.mjs` 断言账本模块不 import 任何开关/闸模块、不发布 ready 字段、`status-western-strings-project.mjs` 不消费账本,并覆盖 schema、`performerId` 必填、consent 必填、未签署不入账、重复 noteId 拒绝与 revision 递增。统计入口 `node scripts/status-western-strings-training-ledger.mjs`,里程碑 `≥300 条 / ≥30 人 / drag 正例 ≥200`,达标也**只**表示可以提议一个预注册的从零训练实验,不授权任何自动采纳;账本明确不得用于调参现有冻结候选(Policy C / Round 5-6)。2026-07-28 已有 1 条真实签署、1 位 performer、`invalidRecords=[]`,学生三开关不变。
...
- **2026-07-21 Oemer dewarp 归因与设计约束:** 在同一冻结 5 页和相同 full-page/up2 输入上重跑原始 staff mask 与 Oemer 内置 dewarp 映射。5/5 映射的 Jacobian fold rate 均为 0；可比较的 3 页 staff-line straightness p90 均改善，但五线组平均恢复率校正前后同为 `85.88%`（1 页改善、1 页恶化、3 页不变），interline CV p90 则 `0/5` 改善、`5/5` 恶化。故“缺少谱线 dewarp 是 onset 5.43% 的主因”被否定：内置 dewarp 有局部直线化收益，却没有稳定恢复谱表完整性。今后任何 dewarp 候选必须同时满足“直线度 p90 不恶化 + interline CV p90 不恶化”，并继续检查五线组恢复率与 Jacobian 折叠；只优化直线度不构成可接受改进。
- **2026-07-21 clean 32 失败构成补审:** `western:m4-clean-failure-modes` 将 render-gold 的 pitch 汇总与逐音结构审计按 piece 精确联结。冻结门槛下 pitch P/R 分别只有 `14/32`、`16/32` 通过，双项通过 `12/32`；onset-quarter、measure 分别只有 `8/32`、`10/32` 通过，四项同时通过仅 `6/32`。尤其 12 张 pitch 严格通过页里仍有 6 张败在 onset 或 measure，因此“照片几何恢复即可复用 clean 100% 能力”的前提不成立：clean 并非 100%，结构重建缺口在无照片退化时已经存在。几何仍可解释照片域的附加损失，但不得再写成唯一根因。
- **2026-07-21 clean 结构深归因:** `western:m4-clean-structure-attribution` 复用冻结评测器的音高序列对齐，逐页输出 predicted−gold onset 的标准差、MAD、步数和首个偏移小节，并核对小节数、逐小节时值和弱起/反复/行内拍号。24 个 onset 失败页中，常量/分段/单调三类已知系统模式只解释 `10/24=41.67%`，其余 `14/24` 为随机型；22 个 measure 失败页中，弱起/反复/行内拍号只解释 `7/22=31.82%`。当前证据不支持“修少数系统 bug 即可过门”，更支持广泛的时值/结构能力缺口。
- **2026-07-21 clean 小节线交叉核对:** `western:m4-clean-barline-crosscheck` 将 22 个 clean measure 失败曲目的 Verovio SVG `barLine` 位置与 Audiveris 原始 `.omr` barline Inter 逐页配对。无漏检页为 `19/22=86.36%`，但无多检页仅 `5/22=22.73%`，完全一一对应仅 `4/22=18.18%`；`17/22` 存在额外小节线。故“小节线 recall 正常”不能推出检测层无责：误检会直接切碎小节，检测与配对/解码必须分开归因，当前不能把主因锁死为纯解码。
- **2026-07-21 解码层上限与止损:** `western:m4-perfect-observation-upper-bound` 从现有 32 首 Verovio SVG 生成精确 staff/symbol/stem-rest/notehead/clef-key 掩膜并直接喂给 Oemer 0.1.8 后处理，跳过 UNet、staff 恢复和 dewarp。29/32 可解码，仍只有 pitch P/R=`59.91%/60.56%`、onset-quarter=`2.70%`、measure=`42.04%`，结构严格通过 `0/29`；因此“检测误差只被放大 2–3%”被否定，按预先分叉进入约束解码 POC。clean-only 硬拍号 DP/候选时值求解仅有 `354/2147=16.49%` 小节可配平，且全部本来已配平、实际改写 `0` 小节；即便使用 gold 拍号边界，onset 也仅到 `10.59%`，32 首严格通过仍为 `0`，远低于 80% 止损线。该解码支线现已停止，不再写 solver、调候选或回头改 dewarp/检测器。
- **2026-07-21 复用 5 真照片一次性结构诊断:** `western:m4-reused-photo-structure-recall` 只重读冻结 Oemer 输出和独立 source-derived MusicXML，不重跑推理、不调阈值。符号关联 recall 为 notehead=`98.27%`、stem=`98.23%`，stem direction=`92.87%`，但 pitch 验证后的 notehead 仅 `76.23%`；解码后的内部 barline boundary recall=`84.71%`。这支持“候选结构大多存活、分类/分组/时值重建仍失真”，但不同版独立源谱无法提供同坐标 mask，故真照片 notehead/stem/barline **像素级** recall 仍明确不可评。报告把本次登记为 formal ledger use `1`，并列出建账前 30 个已知仓库 artifact 作为保守历史基线；这 5 页永久只作诊断，禁止调参和 fresh-blind 晋升。
- **2026-07-21 M4b 决策节点:** 现有 clean 解码线已触发 80% 止损，复用 5 页也已完成唯一一次规定诊断。后续不得继续围绕这 5 页优化或给出晋升结论；能改变 M4b 资格的下一份证据只能是已冻结合同的至少 30 页、6 版式、3 设备 fresh-blind 真照片及逐页结构标注。负责人下一步只需在“采集该 30 页包”与“冻结 M4b 开放域研究线”之间决定；在决定和新数据到位前，`m4bOpenWorldOmrAutomaticAdoptionReady=false`、`m4OmrAutoScoreReady=false` 不变。
- **2026-07-21 SVG 像素 gold 与细结构召回:** M4a MuseScore 构建器现从 SVG 的 `Stem/Beam/Note/BarLine` 语义类直接生成同分辨率四类 mask，并把路径、SHA-256、元素数和前景像素数写入独立 semantic-mask registry；该 registry 与已签署的 M4a 产品 registry/sidecar 分离，后者哈希保持不变。3 个登记页均无 `Beam` 元素，故 beam 明确为不可评测而不是 100%。在 r2-01 clean/synthetic-s17 成对诊断中，Oemer dewarp 同时作用于预测与 gold；2px 容差 recall 分别为 stem `97.37%/96.34%`、notehead `98.02%/98.20%`、barline `93.72%/92.84%`。`stems_rests/symbols` 是非排他合并层，因此这些数字只证明 clean/synthetic 覆盖，不证明类别 precision，也不外推真实照片。它不支持把 clean onset 崩溃归因于基本符干像素漏检；节奏/结构解码仍是更前置的缺口。
- **2026-07-21 ex09/ex10 退化差距审计:** `western:m4-real-degradation-gap` 将两张复用失败照片与冻结 synthetic-test 12 张统一缩放后比较。两页的色度高频 RMS (`0.382/0.343`) 均高于合成最大值 `0.185`，局部墨迹密度 (`0.172/0.164`) 也高于合成最大值 `0.126`；没有一致的 blur、过曝或照明幅度越界证据。下一像素增强已选择 **chromatic screen moire**，但只登记为 future train/calibration 候选，不回写冻结 split；高密度版面必须通过增加公开专业单谱表版式解决，禁止用“把图变黑”伪造内容多样性。
...
- **公开专业单声部 V2 与学生端资格继续分层:** 现有公开专业单声部证据仍只支持 `publicProfessionalMonophonicV2CandidateReady=true` 的研究候选结论。公开数据继续训练或过门也不能自动把 `studentReleaseEligible` 改成 true；学生端合格必须补全全新真实学生录音、逐音人工 gold、独立 fresh-blind 验收和显式发布授权。Round 4 录音已用于当前开发/诊断，不能再充当下一轮独立 fresh-blind；当前缺的是符合新合同且未被查看污染的录音与逐位置 gold，不得以公开专业数据或放宽阈值代签。
- **2026-07-21 ordinary Policy C 已接入教师复核台:** 冻结 12 个故意错误位置上，严格确诊仍为 `2/12` 且非故意位置误指控 `0/253`；把“事件未分配”作为独立的 `self_check_hint` 后增量命中 `4/12`、非故意位置提示 `3/253`，两层复核候选合计 `6/12`。这不是把确诊召回伪写成 6/12：只有 `issue_detected` 可显示为机器确诊候选，其余只能提示自查。`western-round4-policy-c-review-assist-v1` 已持久化进物理候选工件，教师本机队列可显示并在刷新后恢复，候选审计会逐行重算；公网守卫禁止外部访问受控队列，学生响应不含该字段。当前 `policyCReviewAssistRuntime.ready=true`，但 `autoAccusationReady=false`、`energyRobustnessReady=false`、所有学生自动反馈开关仍关闭。drag/extra 的相对 IOI 单阈值在 227 个切分点中没有一个同时达到 precision 90% / recall 50%，下一步必须改做分段 onset 数量与插入/删除结构匹配。

- **2026-07-22 ordinary 确诊召回结构候选审计:** 严格确诊的发布口径仍为 `2/12`。开发集上新增的“相邻替代音被后继同音高长事件合并”结构规则，命中 round4-r4-06 的 1 个 wrong，非故意位置为 `0/253`，所以只得到诊断上限 `3/12`；但该规则是在查看 round4 后提出，13 套外部集合没有独立正例复现，故 `reviewAssistPromotionReady=false`、`automaticAccusationReady=false`。节奏侧完成四条止损：IOI `>0.15`+时长比 `>=1.20` 在两组合成 split 均为 P/R=`95.45%/77.78%`，到完整 round4 降为 `50.00%/66.67%`；通用 onset 峰开发/holdout 为 `42.11%/53.33%`、`22.73%/33.33%`；再加同音高事件门控也只有 `50.00%/53.33%`、`21.05%/26.67%`；未分配事件 edit-path 和内部攻击比分别仅有开发 P/R=`14.29%/6.67%`、`24.00%/80.00%`，均无联合过门点。Policy C 漏音侧的直接 RMS 在合成 split 可达 P=100%，但依赖约 `-134.83dB` 的数字静音，真实 round4 漏音为 `0/3`；目标音高占用率在合成同样 P=100%，到 round4 虽抓 `3/3` 漏音，却把其余 4 个 authoritative assignment gaps 全报错，P/R=`42.86%/100%`。因此能量、目标音高缺失和所有单特征 onset/edit 启发式都不接复核台或学生端；下一轮改为定向采真实混淆对，并训练/验证分段 edit-path 模型，不再追加单阈值。可复跑命令：`npm run western:merged-substitution-candidate`、`npm run western:segment-onset-candidate`、`npm run western:policy-c-waveform-absence`。
- **2026-07-23 教师复核回流入口已进入权威状态:** 已把 Policy C 的“最新物理批机制就绪”与“独立 calibration 复核包有候选”拆开：最新物理批仍为 `mechanismReady=true`、`outputCount=0`，不会伪称当前批有输出；另以冻结 Round 4 报告及其 manifest/truth/候选 SHA 现场重算出的本机包有 9 个可播放候选（2 confirmed + 7 self-check，4 条录音）。`western:project-status` 新增 `ordinaryDynamicShadow.policyCReviewAssistCalibrationPack`，逐行重哈希冻结报告、候选工件、源音频、复制音频、源谱和页面内 ledger 绑定；当前 `sourceCurrent=true`、`readyForReview=true`、`completedReviewPresent=false`。复核结果只能 stage 到私密 calibration 草稿，`freshBlindEligible=false`、`strictConfirmedRecallChanged=false`；未完成人工标签前不计训练数据，完成后也不改变严格 `2/12`。该独立包不写 canonical batch，因而不破坏 M3+ 最新物理批绑定。
- **2026-07-22 temporal operation-path 预闸（Round 5 已判决）:** 已实现 score/event 的 `match/insert/delete/merge/split` 显式动态规划。原始路径在已查看 Round 4 为 `11/12 @ 55/253`，extra 分支贡献 53 FP，故整条原始架构明确拒绝。仅用它精炼 Policy C assignment-gap 自查提示时，Round 4 从 4 TP/3 FP 改善为 4 TP/0 FP；但冻结规则投向 Round 5 fresh-blind 后，gap 自查只得到 `1 TP/1 FP/11 FN`（P/R=`50.00%/8.33%`），missing 严格层为 `1/6 @ 1 FP`（P/R=`50.00%/16.67%`），两者均失败。该首跑不可再用于调参后复考；严格确诊口径仍为 `2/12`，学生自动指控继续关闭。历史预闸证据见 `docs/evidence/western-strings-round5-temporal-operation-path-20260722.json`，首跑报告见 gitignored `data/experiments/western-strings-round5-segment-edit-path/report.json`。
- **2026-07-22 节奏结构合取预闸（Round 5 已判决）:** 注入/旧 Round 4 上的软层曾达到可研究水平；冻结投向 Round 5 fresh-blind 后，soft 与 strict 两层都为 `4 TP/0 FP/8 FN`，P/R=`100%/33.33%`，extra、drag 各命中 `2/6`。零误报保住，但召回低于冻结 `50%` 地板，故两层均不得晋升。
- **2026-07-22 gap 高置信确诊候选（Round 5 已判决）:** recording-level 对齐健康守卫在旧证据上曾压下专业长曲的 595 个原始提示；Round 5 missing 严格层实际为 `1/6 @ 1 FP`，同时违反 precision、recall 与 `FP=0`，候选失败。不得再引用旧 `10/15 @ 0 FP` 合成 holdout 或 Round 4 回顾性上限作为当前晋升依据。
- **2026-07-23 Round 5 intake 与身份审计完成:** `western:round5-targeted-intake` 对 12 条录音、144 个已签署事件、2 人/3 设备/2 房间以及四类各 `12 positive/24 confusion-negative`（fresh 各 `6/12`）全部通过，contract/manifest/truth 绑定为 current。另用不读取 `position-truth.json`、不读取 gate 结果的全谱音高序列交叉匹配审计 cal/fresh 是否对调；12/12 音频均以当前同名谱为第一匹配，全局匈牙利分配也是 12/12 原位，最小第一/第二名成本余量 `0.256`，结论为 `current-mapping-confirmed`，不是 cal/fresh 反置。命令：`npm run western:round5-audio-score-identity`。
- **2026-07-24 Policy C 波形阈值跨域诊断完成:** 在不读取 Round 5 选择阈值、也不重调阈值的前提下，把 r2-01 合成开发集冻结的 RMS `-134.825302dB` 与目标音高占用率 `0` 原样投向 Round 5 的 12 条录音、672 个完整清单谱面位置。RMS 为 `0/12 @ 0/660 FP`，说明它只会识别注入器的数字静音；目标音高缺失为 `5/12 @ 49/660 FP`，P/R=`9.26%/41.67%`，其中 calibration/room-1 为 `4/6 @ 26 FP`，fresh/room-2 仅 `1/6 @ 23 FP`。因此两个单特征都明确未过 `90% precision / 50% recall` 联合地板，`energyRobustnessReady=false`、`targetPitchRobustnessReady=false`，不得接入复核台或学生指控。该包已被查看且房间与 split 完全共线，目标位置还存在谱面上下文混杂，所以它只登记为 consumed diagnostic，`promotionEvidenceEligible=false`；不能把 2 人/3 设备/2 房间误写成独立跨人跨设备 fresh 验收。证据绑定 36 个 manifest/truth/音频/乐谱/合成源/实现文件 SHA，状态节点为 `policyCReviewAssistEvidence.waveformRobustnessDiagnostic`，命令为 `npm run western:round5-policy-c-waveform-robustness`。
- **2026-07-24 当前合同下首个受控试点批已实际执行:** 已按负责人常备批准运行 `western:controlled-pilot-run -- --execute --limit 1`，session `pilot-2026-07-23T16-31-43-427Z` 选中已查看的 `round4-r4-06`。ordinary 全谱 40 行全部保持 `review_required`，shadow 仅作遥测选中 22/40，自动通过、自动反馈和学生发布均为 0；M3+ 对同一物理工件审计为 33 个 `confirmed_center`、0 个 `issue_detected`、7 个 `insufficient_evidence`。Policy C 教师复核层在最新物理批上实际产出 4 个 `self_check_hint`、0 个机器确诊，`policyCReviewAssistRuntime.ready=true`。session 为 `completed_safe`、`pilotRunAccepted=true`、环境恢复且默认学生端仍 fail-closed。由于录音已被反复查看，该运行只证明 review-only 执行链和复核候选可用，状态继续明确 `eligibleAsCurrentReleaseEvidence=false`，不改变严格召回 `2/12`，也不代替 Round 6 untouched fresh。执行还暴露并修复了生命周期缺口：新批次会让旧 M3+ 物理审计失配；runner 现要求执行后依次刷新 release review（含 M3+ physical audit）、decision 和 start preflight，刷新失败即整批不接受。现场补刷后 `physicalEvidenceCurrent=true`、`readyForControlledPilot=true`、`readyToStartControlledPilot=true` 均恢复。
- **2026-07-23 Round 5 片段模型首跑与证据有效性勘误:** gate-specific 固定随机森林按冻结 0.5 决策点完成一次评测；原始数字为 extra `3/6 @ 0/12 confusion FP`、P/R=`100%/50%`，merged substitution `1/6 @ 1 FP`，missing `0/6 @ 0 FP`，drag `3/6 @ 1 FP`。随后不读音频的谱面位置平衡审计发现：calibration 的 merged/missing/drag 以及 fresh 的 merged/extra 标签都可被纯谱面前后音程在 leave-one-recording-out 下完全分开；extra 模型本身还包含这些静态特征。因此 extra 只能记为 `numericallyPromotedGates=["extra"]`，不能作为演奏识别晋升证据；有效 `promotedGates=[]`、`reviewAssistPromotionReady=false`。calibration-only 再审排除谱面上下文特征后，三个失败 gate 均无稳定 performance-evidence 候选过 `90%/50%/0 FP`，当前 calibration 也不能继续调模。下一包必须在录音前运行 `npm run western:round5-position-balance -- --require-ready`，把正例/混淆负例按前一音程、后一音程和片段边界配平；该包的 calibration 与 fresh 都须重采。`automaticAccusationReady=false`、`studentFacing=false`、`productionAdoptionReady=false` 不变。
- **2026-07-23 Round 5 节奏证据有效性补审:** soft/strict 节奏合取的原始 fresh 数字仍是 `4/12 @ 0/312 FP`、P/R=`100%/33.33%`，但 v2 录前位置审计把 extra+drag 正例与完整谱面其余位置统一比较后发现：仅使用书面时值、拍位、相邻音程、重复音、归一化位置和段落边界的静态随机森林，在 calibration 与 fresh 的留一录音中都能做到 `12/12 @ 0/324 FP`。这不证明节奏检测器无效，却证明当前固定目标槽无法隔离“演奏证据”与“谱面位置”；因此 raw `promotionEvidenceEligible=true` 只保留为历史运行器声明，项目状态中的有效值强制为 false，不能再写“节奏 precision 已泛化”。下一包必须跨录音反向轮换 extra/drag 与正常演奏角色，并在录音前通过 v2 全谱位置预检。
- **2026-07-24 Round 6 反向配平候选包与全谱分母守卫就绪:** 新合同保持自动指控原门槛 `P>=90% / R>=50% / strict FP=0`，不采用看过 Round 5 后提出的 recall 20% 降门槛。4 份全新谱各录 3 次；同一位置在三次录音中轮换为正例、混淆负例 A、混淆负例 B。12 条计划共 144 事件、6 个 calibration/fresh 不重叠的演奏者占位、3 设备、4 房间，四 gate 总计各 `12 positive/24 confusion-negative`、fresh 各 `6/12`。v2 录前审计为 `readyForRecording=true`、`confoundedSplitGates=[]`、`rhythmConfoundedSplits=[]`；12 份 PDF 和逐条演奏说明已生成到私密目录。`western:project-status` 现从磁盘重算合同、manifest、truth、材料和分母，并分别 live-bind 位置预检与 intake；`tracks.controlledCandidate.ordinaryDynamicShadow.round6CounterbalancedCapture` 当前为 `readyForRecording=true`、`intakeReady=false`。这里的 12 条只是**最大条件总包**，`readyForRecording` 只表示 12 槽技术设计和材料可用，不是“一次录满 12 条”的调度授权；Round 6 无作用域的 12 条签署入口现已关闭，必须严格走 Stage A calibration 六条和条件 Stage B fresh 六条。评测协议 `western-round6-frozen-evaluation-protocol-v1` 继续固定候选族、0.5 决策点及 `90/50/0` 门槛；严格假阳分母是 fresh 中每个 gate 的全部谱面位置，普通未列位置、其他 gate 正例和其他 gate 对照都计负例。计划见 `docs/western-strings-round6-counterbalanced-capture-plan.md`。
- **2026-07-24 Round 6 候选 v2 去泄漏重冻结:** 录音尚未到位、fresh 仍未消费时完成最后一次输入审计。既有架构烟测已证明固定 RMS/pYIN/onset 堆叠会把已查看 Round 4 从 `5/12 @ 6/253 FP` 降到 `1/12 @ 2/253 FP`，所以没有把“未传 acoustic context”误修成硬叠声学。`western-round6-full-score-candidate-v2` 改为 `full-score-performance-only-random-forest-binary-per-gate`：排除 7 个前后音程/片段边界谱面特征与 11 个固定声学字段，强制保留 8 个 assignment-gap/relative-IOI/时值比/目标窗事件时序字段。执行守卫、协议校验、项目状态和测试均同步检查实际 `featureNames` 不得含禁用字段且不得缺时序字段；加入 Stage B 授权链和冻结模型复用守卫后，评测协议 SHA-256 更新为 `7cd9befaab8461f9f6f3553d716f12068a7af59ace0b1ebcd2bdc5e2ed2094cf`。项目状态中的静态协议层为 `evaluationProtocol.runnerReady=true`、`sourceBindingsCurrent=true`、`reportBindingCurrent=true`、`freshBlindConsumed=false`；实际 Stage B 运行预检因 Stage A 尚未产生通过报告、consumed ledger 和冻结模型而保持 `stageBAuthorizationReady=false`，不会读取 fresh。这只消除取巧与 fresh 重训通道，不构成新的召回数字。
- **2026-07-24 P0–P3 少录音分阶段协议已落成并补齐 fresh 红线:** P1 的 7 个可直接运行候选已全部在真实干净域安全闸淘汰，P2 对 Bach10/URMP/MusicNet 共 5,326 个参考音符的审计得到 0 个独立裁定错误正例、0 组同声部正确/错误演奏对；因此只剩预注册但必须用真实反平衡 calibration 拟合的 `performance-only-RF-v2`。当前不可避免量是 **6 条 calibration**：`r6-cal-a-01/02/03`、`r6-cal-b-01/02/03`；fresh 六条明确未授权。分阶段协议语义 SHA-256=`fe5302fe816d7a436a72e8e40fe8194c28ae05bb694924a7c29102c7f2a07ccb`，绑定 13 个源码/合同/证据来源；项目状态当前为 `recordingSchedule.valid=true`、`stageARecordingAuthorizedNow=true`、`stageBFreshRecordingAuthorizedNow=false`、`currentStageExternalInput.audioFiles=6`、`recordAllTwelveNow=false`。Stage A 链保持 `stage-a-truth-signoff-pack` → scoped apply → position balance → safety preflight → 一次性 safety eval。只有全通过后，Stage B 才可用 `western:round6-stage-b-truth-signoff-pack` 生成 fresh-only 签署包；包与 apply 同时绑定 Stage A 报告、consumed ledger、模型 SHA 和 calibration 不变投影。`western:round6-frozen-eval` 会先写 fresh consumed ledger，再核验 fresh 音频 SHA，并只加载 Stage A 模型；证据必须明确 `trainingPerformed=false`、`frozenModelLoaded=true`，禁止 fresh 期重拟合。任一安全上限超限、绑定失配或崩溃都收线，strict 保持 `2/12`；M4 OMR 与能量验漏音维持停止投入，学生三个开关继续 false/fail-closed。计划见 `docs/western-strings-p3-minimal-recording-plan.md`。
- **2026-07-22 片段模型架构烟测止损:** `western:round5-segment-edit-path-smoke` 用 r2-01 注入作 calibration、r2-08 注入作曲目留出，并只把已查看 Round 4 当真实域诊断。补齐普通位置负例后，五音结构随机森林在 Round 4 为 `5/12 @ 6/253`，P/R=`45.45%/41.67%`；叠加固定 RMS/pYIN/onset 后反降为 `1/12 @ 2/253`，P/R=`33.33%/8.33%`，正确 gate TP 为 0。两者均未过 `90%/50%/0` 联合门槛，`architectureCandidateRetained=false`。因此不再调该模型族或把声学特征硬叠进主线；固定 RF 只作未来真实 Round 5 的基线，下一候选改为显式 temporal operation-path。冻结摘要见 `docs/evidence/western-strings-round5-segment-edit-path-smoke-20260722.json`。
- **M4b 结构 POC 现已完成加压工程闭环:** 四层主链已落地为页面/透视/弯曲归一化、显式五线谱/系统/小节线/小节框/拍号区域证据、确定性结构图冲突解码、以及坐标可归属的多引擎 shadow challenger。旧 synthetic-test 的最大实测弯曲仅 `8.61px`，现已把冻结 12 页提高到绝对 `50.18–59.24px`；单纯放宽曲率上限时整页只有 `7/12`，补上上/下页边双向选优、弱谱线高召回和右端终止小节线保护后，小节框 P/R/F1=`99.55%/99.55%/99.55%`、整页结构 `12/12`、拍号区域 F1=`1.000`、冲突注入 `12/12`。`ready` 还新增期望谱表数绑定与恢复率 `>=90%`，缺期望数或恢复不足均 fail-closed，修复了“只恢复部分谱表仍标 ready”。当前 `m4bStructurePocEngineeringReady=true`、`m4bStructurePocPromotionOperationalReady=true`，但仍只是 `synthetic-engineering-only`；真实 fresh-blind 为 0/30 页、0/6 版式、0/3 设备，所以 `m4bStructurePocPromotionReady=false`、`m4bOpenWorldOmrAutomaticAdoptionReady=false`。
...
- **附录 C 全量现场审计:** `npm run audit:western-appendix-c` 已把两份具名决定、M4a registry/runtime/工程/真实验收执行面、M4b 数据/结构 POC/fresh-blind 执行面、C.2.6–C.2.7 规模化目录以及学生端 fail-closed 边界合并复核。当前 `engineeringComplete=true`、`engineeringBlockingReasons=[]`，但 `appendixAcceptanceComplete=false`。M4a 真实屏拍及负责人 189/189 逐框签署现已完成;唯一附录 C 外部输入仍是 M4b 的至少 30 张/6 版式/3 设备 fresh-blind 照片+结构标注。没有未披露的机器工程 blocker。

- 当前分支为 `feature/model-bakeoff-omr-align`;本节以 2026-07-19 本地 live 复跑和物理产物审计为准,不使用会因本次提交而自指失效的固定提交号。该分支已包含 HOMR 受控离线治理、M3+ 四区 v2 fail-closed 审计、动态闸学生域预考、round-3 reserve/实现验收材料(非发布 fresh-blind)、8 张错版屏拍基准和 10 张登记版本私密屏拍的机器验收。私密原图仍在 git 忽略区;这些变化不改变默认学生端闸门。
...
- 当前项目总闸门要求 ordinary/M3+/M4 三轨同时通过。**2026-07-19 更新(重大):** 负责人已亲口"认"接受 2026-07-18 fresh-blind 证据并"批"过受控试点,`authorizationReady` 已从硬编码 false 改为从负责人常备批准文件(`data/experiments/western-strings-controlled-pilot-approval.json`)派生;ordinary 与 M3+ 两轨现均 `authorizationReady=true`,`m3plus-authorization-closed`/`ordinary-dynamic-shadow-authorization-closed` 均已清除。**2026-07-24 已实际完成一个 review-only、人工复核每一行且不触达学生的受控批；`readyToStartControlledPilot=true` 表示当前证据仍允许后续独立批，不再表示“从未执行”。** 两轨仍各自保留学生端自动反馈闸。M3+ v2 五区证据完整(保护库存 14/14、平拉 gold join `12/12`、揉弦/滑音 join `8/8`,`releaseGateReady=true`)。M4a 完成负责人签署后,M4 必选闸已达标并从项目失败项移除;`projectReleaseReady` 仍为 false,现只由 ordinary/M3+ 学生自动反馈闸保持关闭。(旧值 8/14、0/12、0/8 为 2026-07-17 前的历史状态。)
- **2026-07-19 M4 双轨改绑与真实验收完成:** 负责人已分别签署 M4a/M4b 闸门拆分和 M4b POC 晋升数字冻结。必选 M4 项现改绑 `M4aSupportedEditionRegistrationReady`,不再要求 M4b 开放域 OMR 自动采纳;M4b 的历史失败事实和 `m4bOpenWorldOmrAutomaticAdoptionReady=false` 均保持不变。M4a C.2a-f 工程链已完成并在实现/策略变更后重跑为正例 `3/3`、拒绝例 `4/4`。真实验收的 overlay 崩溃根因是 OpenCV hull 的 `N×1×2` 返回形状被当成 `N×2` 解包,并非照片坐标不足;现已规范化维度并对畸形 polygon fail-closed。10 张登记版本屏拍机器评测为 `10/10` 配准通过,现有 8 张错版/错曲真实屏拍为 `8/8` 拒绝;每张派生 blur/half-page 后为 `20/20` 拒绝、0 漏放。半页安全性通过新增并冻结 `projectedPageVisibility>=0.75` 补强,现场正片最低 0.8155、半页最高 0.3538。负责人签署已绑定 evidence digest `5f8562ffda4df524be26b7a0afd4e470fe40823146c63539e351fe6612287f36`,确认 `189/189`;当前 `operationalReady=true`,`acceptanceReady=true`,`m4aSupportedEditionRegistrationReady=true`,M4a blocking reasons 已清空。
- **2026-07-21 M4b 数据层加压:** 合成结构数据保持 60 张、train/calibration/synthetic-test=`36/12/12`，但曲率合同已改为训练 `0–60px`、标定 `20–60px`、冻结测试 `50–60px`，生成器同时扩大页边安全余量，避免把裁页伪影混进曲率压力结论。5 张 source-gold 与 8 张旧屏拍继续由 live hash ledger 强制 test-only。`m4bDataFoundationReady=true`，但真实结构标签仍 `0/100`，fresh-blind 仍 `0/30`、`0/6` 版式、`0/3` 设备；因此 `m4bFreshBlindDatasetReady=false`，M4b 晋升及自动采纳均保持关闭。
...
- **2026-07-19 更新，2026-07-24 执行:** release review、controlled-pilot decision 与 start preflight 已按当前合同重新生成,且均已转绿:`readyForControlledPilotDecision=true`,`readyToStartControlledPilot=true`,`okToStartControlledPilot=true`。它们绑定当前 live evidence(非缓存假象——`liveEvidenceBindingCurrent=true`),approval 文件 scope 与当前合同精确匹配且持久化安全确认齐全。2026-07-24 已用显式命令完成一个纯 review-only 批；40/40 行仍需人工复核，学生反馈 0。运行后已按新生命周期守卫重绑 M3+ 物理最新批、release review、decision 与 preflight。旧描述(scope 过期/缺确认导致红或“尚未执行”)均已过时。
...
- 当前 v2(2026-07-19 授权接线后):`releaseGateReady=true`,`offlineEvidenceReady=true`,`runtimeFoundationReady=true`,`runtimeAuditReady=true`,`authorizationReady=true`,`readyForMonitoredPilot=true`。2026-07-24 已实际完成一个受控 review-only 批，默认学生闸门仍关闭。离线 probe、gold-free runtime foundation、物理来源 runtime audit、发布授权、试点执行和学生闸门必须分层报告；任何一层不得替代下一层。
...
- `studentGateReady=false`,`m3plusAutoFeedbackReady=false` 保持不变。2026-07-18 的离线证据缺口已全部关闭——6 个 declared-only 保护单元完成实际执行(14/14),12 个平拉来源单元与 8 个揉弦/滑音目标的独立逐音 intonation gold 已按 recording/measure/unit 连接(负责人标注)；2026-07-19 发布授权也已完成，2026-07-24 首个当前合同 review-only 批也已实际执行。当前不再缺执行器、物理审计、授权或“从未运行”的执行证明；剩余边界是该批不具 fresh 晋升资格，以及默认学生运行时按设计保持 fail-closed。
...
清理脚本拒绝工作区外路径和 `paper/` 下任何目标;不会删除正式数据、模型、音频、教师复核、private intake 或论文。当前 M4 监督适配使用的 `clarity-training-audit`、`clarity-train-source-audit`、`clarity-pretrained`、正式 adaptation 数据集/benchmark/训练报告明确保留;只删除具名的 `*-smoke` 调试目录和可再下载的临时下载缓存。`node_modules/.vite` 与 `dist/` 会被清理,发布/验收前必须重新运行 `npm run build`。
