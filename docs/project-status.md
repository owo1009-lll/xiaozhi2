...
- **2026-07-23 Round 5 片段模型首跑:** gate-specific 固定随机森林按冻结 0.5 决策点完成 calibration 训练与 fresh-blind 一次评测。按采集合同“每个子闸独立过门”，`extra` 为 `3/6 @ 0/12 confusion FP`、P/R=`100%/50%`，可冻结为**教师复核候选证据**；`merged_substitution` 为 `1/6 @ 1 FP`（P/R=`50%/16.67%`），`missing` 为 `0/6 @ 0 FP`，`drag` 为 `3/6 @ 1 FP`（P/R=`75%/50%`），三者失败。聚合现明确为 `promotionScope=independent-per-gate`、`promotedGates=["extra"]`，不能因三项失败抹掉 extra，也不能把 extra 外推成四类合格。该 fresh split 已消费；后续只能在 calibration 上诊断失败项，任何改模都必须用新登记的 untouched fresh 包复验。`automaticAccusationReady=false`、`studentFacing=false`、`productionAdoptionReady=false` 不变。
