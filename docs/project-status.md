...
- **2026-07-21 SVG 像素 gold 与细结构召回:** M4a MuseScore 构建器现从 SVG 的 `Stem/Beam/Note/BarLine` 语义类直接生成同分辨率四类 mask，并把路径、SHA-256、元素数和前景像素数纳入 registry live audit。3 个登记页均无 `Beam` 元素，故 beam 明确为不可评测而不是 100%。在 r2-01 clean/synthetic-s17 成对诊断中，Oemer dewarp 同时作用于预测与 gold；2px 容差 recall 分别为 stem `97.37%/96.34%`、notehead `98.02%/98.20%`、barline `93.72%/92.84%`。`stems_rests/symbols` 是非排他合并层，因此这些数字只证明 clean/synthetic 覆盖，不证明类别 precision，也不外推真实照片。它不支持把 clean onset 崩溃归因于基本符干像素漏检；节奏/结构解码仍是更前置的缺口。
