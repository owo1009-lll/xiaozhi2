...
- **2026-07-22 教师复核回流入口:** 已把 Policy C 的“机制就绪”与“当前有候选”拆开：最新物理批次 `mechanismReady=true`，但 `outputCount=0`，因此 `candidateAvailable=false`、`readyForReview=false`，不再把 runtime ready 误写成存在可标候选。另以冻结 Round 4 报告及其 manifest/truth/候选 SHA 现场重算出 9 个可播放候选（2 confirmed + 7 self-check，评测构成为 6 正例/3 混淆负例），生成本机教师复核包。复核结果只能 stage 到私密 calibration 草稿，ledger、音频、谱子均做哈希绑定，且 `freshBlindEligible=false`；未完成人工标签前不计训练数据，完成后也不改变严格 `2/12`，只作为真实混淆对种子。
- **2026-07-22 temporal operation-path 预闸:** 已实现 score/event 的 `match/insert/delete/merge/split` 显式动态规划。原始路径在已查看 Round 4 为 `11/12 @ 55/253`，extra 分支贡献 53 FP，故整条原始架构明确拒绝。仅用它精炼 Policy C assignment-gap 自查提示时，Round 4 从 4 TP/3 FP 改善为 4 TP/0 FP；加原有 2 个严格确诊后，两层回顾性上限为 **6/12 @ 0/253**；r2-01/r2-08/r3-01~03 五条自然正确录音压力测试也保持 0 FP。随后按公开专业独奏输入契约补测 Oliver Colbentson 的两段 BWV1006（反复记号先展开，合计 2,301 个谱音），仍产生 936 个 assignment gap 和 595 个精炼提示（258.58/千音）。该公开集没有逐音演奏错误人工 gold，因此 595 只能称“可观察复核负担”，不能冒充 595 个 FP；但负担已足以否决通用长曲部署，`generalPurposeCandidateRetained=false`。短篇学生域候选现已用冻结参数和冻结规则接入 Round 5 targeted runner；每条真值必须声明 `completeErrorInventory=true`，否则未列位置不能当正确负例、intake 直接 fail-closed。当前缺 manifest/truth，所以 `runnerWired=true`、`evaluationPerformed=false`、`reviewAssistPromotionReady=false`；数据到位后可直接首跑 untouched fresh-blind，无需再改代码或调参。该规则是在看过 Round 4 后形成，所以 `strictConfirmedRecallUnchanged=true`、严格口径仍 `2/12`，只输出 `self_check_hint`，不接自动指控。权威证据及全部源 SHA 绑定见 `docs/evidence/western-strings-round5-temporal-operation-path-20260722.json`。
- **2026-07-22 节奏结构合取预闸:** 单独的 relative-IOI/时值/onset 特征仍维持失败结论；新增候选只在 relative-IOI `>0.15`、时值比 `>=1.20`、事件置信度 `>=0.75` 且冻结 operation-path 在同位置产生候选时输出 `self_check_hint`。注入 calibration 为 `18 TP/1 FP/9 FN`（P/R=`94.74%/66.67%`），曲目 holdout 为 `15/27 @ 0 FP`（P/R=`100%/55.56%`）；已查看 Round 4 为 `4/6 @ 0 FP`（drag `3/3`、extra `1/3`），五条自然正确录音为 `5/285=1.75%` 提示负担，低于既有 2% 自查提示负担线。该规则是在旧证据已查看后形成，以上数字全部只是诊断，`promotionEvidenceEligible=false`、`automaticAccusationReady=false`，严格确诊仍为 `2/12`。冻结规则现已作为 `extra`/`drag` 子闸接入同一个 Round 5 targeted runner；当前因私密 manifest/truth 未到位而保持 `evaluationPerformed=false`，只能由 untouched fresh-blind 首跑决定是否进入教师复核。
- **2026-07-22 高置信节奏确诊候选:** 在上述四信号合取内把时值比提高到 operation-path 已有冻结值 `>=1.30`，形成独立 `issue_detected_candidate`：注入 calibration `18/27 @ 0 FP`（P/R=`100%/66.67%`），曲目 holdout `14/27 @ 0 FP`（P/R=`100%/51.85%`），已查看 Round 4 `4/6 @ 0 FP`（P/R=`100%/66.67%`），五条自然正确录音 `0/285`。与原有 2 个严格确诊合并的回顾性严格候选上限为 **6/12 @ 0/253**；再加入 4 个 gap 自查提示，三层复核上限为 **10/12 @ 0/253**。公开专业长曲 2,301 个谱音仍产生 13 个未裁决候选（5.65/千音）；没有逐音演奏错误 gold，不能冒充 13 FP，但通用自动指控因此不获授权。该合取是在查看当前证据后形成，故 `automaticAccusationEvidenceReady=false`、`automaticAccusationReady=false`、正式确诊仍 `2/12`。高置信层已与软提示层一起接入 Round 5 targeted runner；只有 complete-inventory untouched fresh-blind 过 `P>=0.90/R>=0.50/FP=0` 才能形成晋升证据。
- **2026-07-22 gap 高置信确诊候选:** 原始 gap 精炼在两段专业长曲产生 595 个提示，不能直接升级。新增的不是逐音置信度调参，而是 recording-level fail-closed 对齐健康守卫：assignment gap 必须 `<=5` 且 gap rate `<=10%`，否则整条只报证据不足。守卫后的 missing-targeted `issue_detected_candidate` 在 calibration/holdout 均命中 `10/15 missing @ 0 strict FP`，已查看 Round 4 命中 `3/3 missing` 加 1 个额外真实 wrong、仍为 `0/253`，五条自然正确录音 0 候选；两段专业长曲因 gap rate=`47.65%/23.57%` 被拒绝，595 个原始提示不再升级为指控。它与既有严格 2 个及节奏严格候选 4 个合并，回顾性严格候选上限为 **10/12 @ 0/253**。守卫是在查看当前证据后形成，所以该数字不改变正式 `2/12`；`automaticAccusationEvidenceReady=false`、`automaticAccusationReady=false`，只有 untouched Round 5 可晋升。
- **2026-07-22 Round 5 定向诊断 intake 已纳入总状态:** `western:round5-targeted-intake` 现同时绑定 contract/manifest/truth SHA-256；`western:project-status` 在 `ordinaryDynamicShadow.round5TargetedIntake` 实时检查报告是否与磁盘一致。当前 `ready=false`、`bindingCurrent=true`，不是工具或报告过期，而是 `data/private/western-strings-round5/manifest.csv` 与 `position-truth.json` 尚未建立。任一输入后到或变更而未重跑 intake 时会显式产生 `*-binding-stale`，学生端与自动授权仍固定为 false。
- **2026-07-22 Round 5 采集包已可执行:** `docs/round5-targeted-diagnosis-capture-pack/index.html` 冻结 12 条录音槽位，calibration/fresh-blind 各 6 条；每条四类各 1 个正例、各 2 个独立混淆负例，总分母恰为每类 `12 positive/24 negative`，fresh-blind 每类 `6/12`，并覆盖 2 人、3 设备、2 房间且 split 上下文不重叠。页面可下载 manifest/truth，`western:round5-capture-pack` 可确定性重建模板；模板默认 `completeErrorInventory=false` 且位置为空，不会误过闸。intake 新增非法位置和同录音重复 `measure/beat/scoreMidi` 拒绝，禁止复制一个位置凑多个分母。当前真正剩余的是录制全新曲目、逐音填写 144 个互不重复事件槽并人工签署完整错误库存。
- **2026-07-22 Round 5 片段 edit-path 模型入口完成:** `western:round5-segment-edit-path` 已把目标音前后各两音的 assignment gap、音高替代、relative-IOI、时值与未分配事件组合为 gate-specific segment 特征；只允许 calibration 训练、fresh-blind 固定 0.5 决策点评测，不在盲测集选参。总状态会同时校验训练报告的 contract/manifest/truth 绑定与模型工件 SHA-256。当前因 Round 5 私密输入未到位，权威结果是 `trainingPerformed=false`、`reviewAssistPromotionReady=false`，没有生成或假装出一个过拟合模型；学生端、自动指控与生产采纳均保持 false。
