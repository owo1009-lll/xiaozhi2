...
- **2026-07-22 ordinary 确诊召回结构候选审计:** 严格确诊的发布口径仍为 `2/12`。开发集上新增的“相邻替代音被后继同音高长事件合并”结构规则，命中 round4-r4-06 的 1 个 wrong，非故意位置为 `0/253`，所以只得到诊断上限 `3/12`；但该规则是在查看 round4 后提出，13 套外部集合没有独立正例复现，故 `reviewAssistPromotionReady=false`、`automaticAccusationReady=false`。节奏侧也完成两条止损：相对 IOI `>0.15` 且时长比 `>=1.20` 在 r2-01 合成开发和 r2-08 合成 holdout 均为 P/R=`95.45%/77.78%`，到完整 round4 却降为 `50.00%/66.67%`；通用波形起音峰在 150 组参数中无一达到 P>=90%/R>=50%，开发最优 P/R=`42.11%/53.33%`，搬到合成 holdout 仅 `22.73%/33.33%`。因此两者都不接复核台或学生端；下一候选收窄为“音高条件化的重复起弓/插入删除 edit-path”，并必须取得全新独立正例和 position-labelled fresh-blind 证据。可复跑命令：`npm run western:merged-substitution-candidate`、`npm run western:segment-onset-candidate`。
