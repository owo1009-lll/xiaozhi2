...
- **附录 C 全量现场审计:** `npm run audit:western-appendix-c` 已把两份具名决定、M4a registry/runtime/工程/真实验收执行面、M4b 数据/结构 POC/fresh-blind 执行面、C.2.6–C.2.7 规模化目录以及学生端 fail-closed 边界合并复核。当前 `engineeringComplete=true`、`engineeringBlockingReasons=[]`，但 `appendixAcceptanceComplete=false`。这一 false 仅对应两类真实输入:M4a 的 10 张精确版本屏拍+后续逐框签署，以及 M4b 的至少 30 张/6 版式/3 设备 fresh-blind 照片+结构标注;没有未披露的机器工程 blocker。
