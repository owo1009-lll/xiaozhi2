...
- **2026-07-22 temporal operation-path 预闸:** 已实现 score/event 的 `match/insert/delete/merge/split` 显式动态规划。原始路径在已查看 Round 4 为 `11/12 @ 55/253`，extra 分支贡献 53 FP，故整条原始架构明确拒绝。仅用它精炼 Policy C assignment-gap 自查提示时，Round 4 从 4 TP/3 FP 改善为 4 TP/0 FP；加原有 2 个严格确诊后，两层回顾性上限为 **6/12 @ 0/253**；r2-01/r2-08/r3-01~03 五条自然正确录音压力测试也保持 0 FP。该规则是在看过 Round 4 后形成，所以 `strictConfirmedRecallUnchanged=true`、严格口径仍 `2/12`，不接运行时；只登记 `gapRefinementCandidateRetainedForFreshBlind=true`，等待全新 Round 5 包原样验证。权威证据及全部源 SHA 绑定见 `docs/evidence/western-strings-round5-temporal-operation-path-20260722.json`。
