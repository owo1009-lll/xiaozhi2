...
- **2026-07-24 Round 6 反向配平候选包与全谱分母守卫就绪:** 新合同保持自动指控原门槛 `P>=90% / R>=50% / strict FP=0`，不采用看过 Round 5 后提出的 recall 20% 降门槛。4 份全新谱各录 3 次；同一位置在三次录音中轮换为正例、混淆负例 A、混淆负例 B。12 条计划共 144 事件、6 个 calibration/fresh 不重叠的演奏者占位、3 设备、4 房间，四 gate 总计各 `12 positive/24 confusion-negative`、fresh 各 `6/12`。v2 录前审计为 `readyForRecording=true`、`confoundedSplitGates=[]`、`rhythmConfoundedSplits=[]`；12 份 PDF 和逐条演奏说明已生成到私密目录。`western:project-status` 现从磁盘重算合同、manifest、truth、材料和分母，并分别 live-bind 位置预检与 intake；`tracks.controlledCandidate.ordinaryDynamicShadow.round6CounterbalancedCapture` 当前为 `readyForRecording=true`、`intakeReady=false`。这里的 12 条音频、12 条同意、12 条许可、144 个 `asPerformed` 和 12 个 `completeErrorInventory` 是**最大条件总包**，`readyForRecording` 只表示 12 槽技术设计和材料可用，不是“一次录满 12 条”的调度授权；当前准确录音范围由同节点的 `recordingSchedule` 决定。完整 12 条兼容流程仍保留：`western:round6-truth-signoff-pack` 只有 12 条音频全部存在时才生成无机器预测的逐条试听页，绑定每条音频 SHA，并在下载前强制 144 个事件、12 个完整清单和 12 组实际录音元数据全部签署。下载产物还绑定合同/manifest/truth/逐音频 SHA；`western:round6-truth-signoff-apply` 默认 dry-run，只有 source hashes、ID 集、位置、授权、6/3/4 覆盖及 cal/fresh 演奏者隔离全部通过后，显式 `--apply` 才会同时更新私密 truth/manifest 并留下旧哈希备份。评测也已在音频到位前冻结：`western-round6-frozen-evaluation-protocol-v1` 绑定执行守卫、全谱候选 runner、历史特征提取器、temporal-operation 规则、intake validator、音频分析器和合同，固定候选族、0.5 决策点及 `90/50/0` 门槛；严格假阳分母现明确为 fresh 中每个 gate 的全部谱面位置，除该 gate 签署正例外，普通未列位置、其他 gate 正例和其他 gate 对照都计负例。完整 intake 绿后 `western:round6-frozen-eval` 只允许一次，开始读 fresh 前即写 consumed ledger；崩溃也不得重跑，任何数值通过仍不自动授权学生端。计划见 `docs/western-strings-round6-counterbalanced-capture-plan.md`。
- **2026-07-24 Round 6 候选 v2 去泄漏重冻结:** 录音尚未到位、fresh 仍未消费时完成最后一次输入审计。既有架构烟测已证明固定 RMS/pYIN/onset 堆叠会把已查看 Round 4 从 `5/12 @ 6/253 FP` 降到 `1/12 @ 2/253 FP`，所以没有把“未传 acoustic context”误修成硬叠声学。`western-round6-full-score-candidate-v2` 改为 `full-score-performance-only-random-forest-binary-per-gate`：排除 7 个前后音程/片段边界谱面特征与 11 个固定声学字段，强制保留 8 个 assignment-gap/relative-IOI/时值比/目标窗事件时序字段。执行守卫、协议校验、项目状态和测试均同步检查实际 `featureNames` 不得含禁用字段且不得缺时序字段；Stage A 增加 calibration-only 数据过滤后重新冻结评测协议 SHA-256=`7546020e1e0f326910b160c7bca0f12602c40bce9477efd764c4f23e2f4e3d75`。现场状态为 `runnerReady=true`、`sourceBindingsCurrent=true`、`reportBindingCurrent=true`、`freshBlindConsumed=false`；这只消除取巧通道，不构成新的召回数字。
- **2026-07-24 P0–P3 少录音分阶段协议已落成并取代一次录满 12 条的旧调度:** P1 的 7 个可直接运行候选已全部在真实干净域安全闸淘汰，P2 对 Bach10/URMP/MusicNet 共 5,326 个参考音符的审计得到 0 个独立裁定错误正例、0 组同声部正确/错误演奏对；因此只剩预注册但必须用真实反平衡 calibration 拟合的 `performance-only-RF-v2`。当前不可避免量是 **6 条 calibration**：`r6-cal-a-01/02/03`、`r6-cal-b-01/02/03`；fresh 六条明确未授权。分阶段协议语义 SHA-256=`d08bff4b76a114feaf93f4d2d00fb2e37df54660d67b0edc7d6fc1f313777a3f`，绑定 12 个源码/合同/证据来源；项目状态当前为 `recordingSchedule.valid=true`、`stageARecordingAuthorizedNow=true`、`stageBFreshRecordingAuthorizedNow=false`、`currentStageExternalInput.audioFiles=6`、`recordAllTwelveNow=false`。Stage A 无预测签署链为 `western:round6-stage-a-truth-signoff-pack` → scoped dry-run/`--apply` → `western:round6-position-balance` → `western:round6-stage-a-safety-preflight` → 一次性 `western:round6-stage-a-safety-eval`；它只读取 calibration，要求 fresh 音频不存在，并把签署 lineage、模型、报告和 consumed ledger 全部 live-bind。任一安全上限超限、证据失配或执行崩溃都收线并省掉 fresh 六条，strict 保持 `2/12`；只有 Stage A 全通过才允许补 6 条 untouched fresh。M4 OMR 与能量验漏音维持停止投入，学生三个开关继续 false/fail-closed。计划见 `docs/western-strings-p3-minimal-recording-plan.md`。
