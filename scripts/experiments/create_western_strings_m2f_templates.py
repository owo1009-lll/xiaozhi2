    ...
    readme.write_text(
        "\n".join(
            [
                ...
                "   Repo-local student audio and local-only private score files must live",
                "   under `data/private/...`; outside-repo private local paths are also allowed.",
                ...
                "5. Run `npm run western:m2f-manifest-status` before result counting.",
                "6. Run the studentSafe preview and human/gold review, then fill",
                "   `real-student-recording-results.csv` counts.",
                "7. Use `npm run western:m2f-status` for inspection.",
                "8. Use `npm run western:m2f-gate` as the release-blocking command.",
                "",
