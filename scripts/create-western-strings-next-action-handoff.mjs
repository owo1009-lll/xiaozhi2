  ...
  if (track === "M2/M3 ordinary upload candidate gate") {
    ...
    if (hasReason(action, "ordinary-auto-gate-disabled-by-default")) {
      return [
        ...
        "Run npm run western:ordinary-monitored-pilot-plan after the smoke and precision precheck; the plan now fails closed if the precheck is missing, has known-wrong rows, or has unknown rows still needing review.",
