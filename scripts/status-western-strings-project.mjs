    ...
    actions.push({
      ...
      action: round5Evaluated
        ? round6ReadyForRecording
          ? "Round 5 is consumed and cannot support promotion because its apparent gate and rhythm gains are score-position confounded. The Round 6 counterbalanced contract, 12-take manifest, 144-event truth, capture materials, and v2 position preflight are now hash-current and ready for recording. Record exactly the 12 specified takes without changing the positive/negative role rotations. After all audio files are present, run `npm run western:round6-truth-signoff-pack`; its no-machine-prediction page binds every audio SHA and requires all 144 `asPerformed` fields plus 12 complete error inventories before download. Apply the reviewed completed truth, mark consent and local-only licensing in the manifest, then run `npm run western:round6-targeted-intake`. Strict confirmed recall remains 2/12 and every student automatic-accusation path stays closed until a new untouched fresh evaluation passes the frozen 90% precision / 50% recall / 0 strict-FP gate."
